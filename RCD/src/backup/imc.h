/* $Id: imc.h,v 1.15 2000/02/29 11:56:15 gmoody Exp $ */
/*
...
#define REVISION "Revision 0 2/29/2000"
/* activation states */
...
/* HEAP_ALLOC is used for lzo compression. Put it here for now -- Kratas */
...
typedef struct
{
  ...
  long txcompressed_bytes;      /* Transmitted bytes that are compressed */
  long txexpanded_bytes;        /* When fully expanded...                */
  long rxcompressed_bytes;      /* Recieved bytes that are compressed    */
  long rxexpanded_bytes;        /* When fully expanded...                */
...
//void imc_cancel_event_c(void (*callback)(void *), void *data);
...
void imc_recv_tell(const imc_char_data *from, const char *to,
		   const char *argument, int isreply);
void imc_recv_whoreply(const char *to, const char *data, int sequence, int inlen);
