...
/*  gettimeofday( &boot_time, NULL);   okay, so it's kludgy, sue me :) */
    ...
    log_string("Booting Database"); boot_db(fCopyOver );
#ifdef I3
...
/* i need this for debugging - shogar */
	to_channel( log_buf, CHANNEL_MONITOR, "Monitor", 113);
	show_title(d);
	break;
...
/*	DISPOSE( ch->pcdata->pwd ); */
	...
	{
	write_to_buffer( d, "Please choose a vaild hometown from the list below:\n\r", 0 );
	write_hometown_list( d );
...
/*
      ==========================
      Hometown Char-gen Addition 12/3/01 -Nopey
      ==========================
*/
      case CON_GET_HOMETOWN:
      {
       ...
       if( (argument[0] == '\0')
       || ((htown = hometown_lookup(argument)) == NULL) )
       {
	 write_to_buffer( d, "That's not a vaild hometown.\n\r", 0 );
	 write_to_buffer( d, "\n\rPlease choose a vaild hometown from the list below:\n\r", 0 );
	 write_hometown_list( d );
	 d->connected = CON_GET_HOMETOWN;
	 break;
       }
	  ...
	  /* send to ansi check    -Nopey */
	  write_to_buffer( d, "\n\rWould you like RIP, ANSI or no graphic/color support, (A/N)? ", 0 );
...
/*
	   =================================
	   Update characte hometown on login  -Nopey
	   =================================
	   */
	   if( ch->pcdata->htown_name && ch->pcdata->htown_name[0] != '\0' 
)
	   {
	     HOMETOWN_DATA *chtown = NULL;

	     if((chtown = hometown_lookup(ch->pcdata->htown_name)) == NULL)
	     {
	       sprintf( buf, "%s invaild hometown %s.\n\r", ch->name, ch->pcdata->htown_name );
	       bug( buf, 0 );
	       STRFREE( ch->pcdata->htown_name );
	       ch->pcdata->hometown	       = NULL;
	     }
	     else
	       ch->pcdata->hometown	       = chtown;

	       /* some memory management 12/5/01 */
	       free_hometown(chtown);
	    }
#ifdef I3
   I3_char_login( ch );
#endif
    ...
    if (victim->max_hit > 0)
      ...
      case 'i':
        if ( (!IS_NPC(ch) && xIS_SET(ch->act, PLR_WIZINVIS)) ||
          ...
          sprintf(pbuf, "(Invis %d) ", (IS_NPC(ch) ? ch->mobinvis
: ch->pcdata->wizinvis));
        else
...
void do_crashguard ()
{
  ...
  if (!fp)
   ...
   /*
    * Looks like Josh forgot this in his condensed version of copyover
    *					--Callidyrr
    */
...
if (sysdata.webtoggle==TRUE)
{
   ...
   if( I3_is_connected() )
   {
    ...
    fclose (fp);
        ...
        execl (CRASHSAFE, "rmexe", buf, "copyover", buf2, buf3,
          buf4, buf5, (char *) NULL);
...
void sig_handler(int sig)
{
	...
	switch(sig)
  {
    case SIGBUS:
      ...
      do_crashguard();
      log_string_plus( "SIG:HANDLER:(BUS) similar to a segment violation", LOG_NORMAL, LEVEL_GREATER ); 
      break;
    case SIGTERM:
      log_string_plus( "SIG:HANDLER:Terminal Shutdown", LOG_NORMAL, LEVEL_GREATER );
      bug("Sig handler SIGTERM.",0);
      do_crashguard();
      break;
    case SIGABRT:
      log_string_plus( "SIG:HANDLER:Abort", LOG_NORMAL, LEVEL_GREATER );
      bug("Sig handler SIGABRT.",0);
     /* do_crashguard();
     */ break;
    case SIGSEGV:
      log_string_plus( "SIG:HANDLER:Segment Violation", LOG_NORMAL, LEVEL_GREATER );
