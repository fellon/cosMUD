/* $Id: iced.c,v 1.18 2000/01/29 18:55:18 deaz Exp $ */
/*
...
/* make these  global and move to imc-interp for 2.00 - shogar - 1/25/2000 */
extern ice_channel *iced_channel_list;
extern ice_channel *next_refresh; 

static ice_channel *next_list;
...
static void fixactive(ice_channel *c)
{
  ...
  if (c->policy != ICE_PRIVATE && c->policy != ICE_CPRIVATE)
  {
...
int iced_recv(const imc_packet *p, int bcast)
{
    ...
    else if( !strcasecmp( p->type, "ice-comp-msg-p" ) )
    {
        iced_recv_msg_comp_p( p->from,
                              imc_getkey( &p->data, "channel", "" ),
                              imc_getkeyi( &p->data, "complen", 0 ),
                              imc_getkey( &p->data, "text", "" ),
                              imc_getkeyi( &p->data, "emote", 0 ));
        return 1;
    }
    ...
    // Let iced_recv_msg_b handle below call
    else if( !strcasecmp( p->type, "ice-comp-msg-b" ) )
    {
        iced_recv_msg_b( p->from,
                         imc_getkey( &p->data, "channel", "" ) );
        return 1;
    ...
    /* Get rid of this puppy so they can't use it to jam the network
     for 2.00 - shogar - 1/26/2000
...
/* gone for 2.00 - shogar - 1/26/2000 */
...
void iced_policy(ice_channel *c, const char *cname, const char *from, const char *data)
{
    ...
    /* added compress policy - shogar - 1/29/2000 */
    if (!strcasecmp(data, "copen"))
    {
        c->policy=ICE_COPEN;
        iced_announce(c, "Channel policy is now: open with compression.");
        iced_gannounce("%s is now policy: open.", c->name);
        iced_update(c, NULL);
    }
    ...
    /* added compress policy - shogar - 1/29/2000 */
    else if (!strcasecmp(data, "cprivate"))
    {
        c->policy=ICE_CPRIVATE;
        iced_announce(c, "Channel policy is now: private with compression.");
        iced_gannounce("%s is now policy: private.", c->name);
        iced_update(c, NULL);
    }
    else
        imc_send_tell(&iced_char, from, "Syntax: <channel> policy [open|copen|closed|private|cprivate]", 1);
}
...
void iced_refresh(ice_channel *c, const char *cname, const char *from, const char *data)
{
  if (!c)
  {
	...
	next_list=iced_channel_list;
        /* 2.00 fix changed from ev_chan_list to ev_refresh - shogar - 
	1/25/2000 */
        imc_cancel_event(ev_iced_refresh,NULL); 
	imc_add_event(30, ev_iced_refresh, (void *)from, 1);
      }
      else
      {
...
/* Updated function, does compression */
void iced_recv_msg_comp_p(const char *from, const char *chan, int complen,
                          const char *txt, int emote)
{
   ...
   /* added compress policy - shogar - 1/29/2000 */
    ...
    strcpy(out.from, "ICE");
    strcpy(out.type, "ice-comp-msg-r"); /* redirect */

    imc_initdata(&out.data);
    imc_addkey(&out.data, "realfrom", from);
    imc_addkey(&out.data, "channel", chan);
    imc_addkeyi( &out.data, "complen", complen );
    imc_addkey(&out.data, "text", txt);
    imc_addkeyi(&out.data, "emote", emote);
...
void iced_recv_refresh(const char *from, const char *chan)
{
  if (!strcmp(chan, "*"))
  {
    ...
    {
      next_list=iced_channel_list;
        /* 2.00 fix changed from ev_chanlist to refresh - shogar - 1/25/2000 */
      imc_cancel_event(ev_iced_refresh,NULL); 
      imc_add_event(30, ev_iced_refresh, (void *)from, 1);
    }
    else
...
void iced_update(ice_channel *c, const char *to)
{
  ...
  imc_addkey(&out.data, "policy",polly);
  imc_addkey(&out.data, "invited", c->invited);
  imc_addkey(&out.data, "excluded", c->excluded);

  imc_send(&out);
  imc_freedata(&out.data);
  imc_logstring("Channel update");
  imc_logstring(out.to);
}

/* spam out all channel updates at once */
/* not using anymore for 2.00 - shogar - 1/25/2000 */
void ev_iced_chanlist(void *data)
{
  ...
  imc_cancel_event(ev_iced_chanlist,NULL); /* 2.00 fix - shogar - 1/25/2000 */
  imc_add_event(30, ev_iced_chanlist, data, 1);
...
/* removed so it doesnt recycle automatically for 2.00 - shogar - 1/25/2000 */
/*
  if (!next_refresh)
    next_refresh=iced_channel_list;
*/
...
/* copied this down here for 2.00 , avoid useless event  - shogar - 1/25/2000 */
