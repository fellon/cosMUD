 ...
 * quoted text. */
char *break_fellon(char *str, char *first_arg, int bufsize, int max, int *buflen, int *len)
{
    int slen=0;
    char *arg;
...
char *string_justify(char *str, int len, int width, int numwords, int *rlen)
{
    ...
    {
        str = break_fellon(str, arg, sizeof(arg), width, &alen, NULL);
        strcpy(bp, arg);
        bp += alen;
...
char *string_format(char *str, int *numlines, int width, int height, bool unjust)
{
    ...
    --width;
    *pret='\0';
    for (sp = break_fellon(str, arg, sizeof(arg), width, &len, &alen); *arg;
         sp = break_fellon(sp, arg, sizeof(arg), width, &len, &alen))
    {
        blen += alen;
...
/*    {
        size = IS_IMMORTAL( ch ) ? size :
            get_curr_int( ch )/2 +
            ( ( !IS_NPC( ch ) ) ? ch->pcdata->learned[gsn_spy]/25 : 0 ); 
        if ( size % 2  == 0 )
            size +=1;
