/* $Id: imc-mercbase.c,v 1.27 2000/02/20 13:19:50 gmoody Exp $ */
/*
...
#include "imc.h"
#include "minilzo.h" /* compressed who - shogar - 1/24/2000 */
#include "imc-mercbase.h"

extern int imc_log_on;
extern int compressit; /* compress who - shogar - 1/24/2000 */

int silent_rtell=0;

/* LZO Decompression Wrapper */
int lzo_decompress( const lzo_byte *, lzo_uint, lzo_byte *, lzo_uint *, lzo_voidp );

#ifdef CRBS
void imc_recv_chat_command(const char *from, const char *to,
...
#endif
void ev_iced_refresh(void *dummy) /*dummy catch cause muds has no iced.c-shogar */
{
}
...
#define RTELL_FORMAT_1 "~cYou rtell ~C%s ~c'~W%s~c'~!\n\r"
#define RTELL_FORMAT_2 "~C%s ~crtells you ~c'~W%s~c'~!\n\r"
#define RBEEP_FORMAT_1 "~cYou rbeep ~Y%s~c.~!\n\r"
#define RBEEP_FORMAT_2 "~c\a%s rbeeps you.~!\n\r"

/*  maps IMC standard -> mud local color codes
 *  let's be unique, noone uses ~ :>
 */

/* Modify the defines in imc-color.h. Will be replaced eventually. -- Kratas */
struct {
...
trans_table[]=
{
    ...
    { "~!", IMC_NORMAL },  /* reset */
    { "~d", IMC_NORMAL },  /* default */

    { "~b", IMC_BLUE },  /* dark blue */
    { "~g", IMC_GREEN },  /* dark green */
    { "~r", IMC_RED },  /* dark red */
    { "~y", IMC_BROWN },  /* dark yellow / brown */
    { "~m", IMC_MAGENTA },  /* dark magenta / purple */
    { "~p", IMC_PURPLE }, /* dark magenta / purple ? */
    { "~c", IMC_CYAN },  /* cyan */
    { "~w", IMC_GREY },  /* white */

    { "~D", IMC_DARK_GREY },  /* dark grey / "z" ? */
    { "~B", IMC_BOLD_BLUE },  /* light blue */
    { "~G", IMC_BOLD_GREEN },  /* light green */
    { "~R", IMC_BOLD_RED },  /* light red */
    { "~Y", IMC_YELLOW },  /* light yellow */
    { "~M", IMC_BOLD_MAGENTA },  /* light magenta / purple */
    { "~C", IMC_BOLD_CYAN },  /* light cyan */
    { "~W", IMC_WHITE },  /* bright white */
    ...
    { "~!", "@@i" },  /*  ditto  */
    { "~D", "@@k" },  /* black -> grey */
    { "~!", "@@0" },  /* catch background color codes */
    { "~!", "@@1" },  /*  ditto  */
    { "~!", "@@2" },  /*  ditto  */
    { "~!", "@@3" },  /*  ditto  */
    { "~!", "@@4" },  /*  ditto  */
...
DEFINE_DO_FUN(do_rwho)
{
  ...
  CHECKMUD(ch, arg);
  /* changed "who" to "wHo" compressed who - shogar - 1/24/2000 */
...
DEFINE_DO_FUN(do_rbeep)
{
  ...
  sprintf(buf, color_itom(RBEEP_FORMAT_1), argument);
  imc_to_char(buf, ch);
...
DEFINE_DO_FUN(do_imclist)
{
...
#endif
    if (!argument[0])
        imc_to_pager(color_itom(imc_list(3)), ch);
    else if (!str_prefix(argument, "all"))
        imc_to_pager(color_itom(imc_list(6)), ch);
    ...
    else if (!str_prefix(argument, "direct"))
    {
        if (get_trust(ch)>=MAX_LEVEL-1)
            imc_to_pager(color_itom(imc_list(2)), ch);
        else if (IS_IMMORTAL(ch))
            imc_to_pager(color_itom(imc_list(1)), ch);
        else
            imc_to_pager(color_itom(imc_list(0)), ch);
    }
    else if (!str_prefix(argument, "config"))
    {
        if (IS_IMMORTAL(ch))
            imc_to_pager(color_itom(imc_list(5)), ch);
        else
            imc_to_pager(color_itom(imc_list(4)), ch);
    }
    else
...
DEFINE_DO_FUN(do_rsockets)
{
  ...
  imc_to_pager(color_itom(imc_sockets()), ch);
}
...
DEFINE_DO_FUN(do_imc)
{
    ...
    {   // Temporary color_itom here... Eventually will get abetter way.--Kratas
        imc_to_char(color_itom("Syntax:\n\r"
                    "~c         imc add          <~!mudname~c>\n\r"
                    "         imc delete       <~!mudname~c>\n\r"
                    "         imc set          <~!mudname~c> all <~!host~c> <~!port~c> <~!clientpw~c> <~!serverpw~c>\n\r"
                    "                          <~!rcvstamp~c> <~!noforward~c> <~!flags~c>\n\r"
                    "         imc set          <~!mudname~c> host|port|clientpw|serverpw|rcvstamp|\n\r"
                    "                                    noforward|flags <~!value~c>\n\r"
                    "         imc rename       <~!oldmudname~c> <~!newmudname~c>\n\r"
                    "         imc localname    <~!mudname~c>\n\r"
                    "         imc localport    <~!portnum~c>\n\r"
                    "         imc log (toggles all log messages)\n\r"
                    "         imc hubswitch (toggles hubswitching - for firewalls)\n\r"
                    "         imc info name    <~!LocalName~c>\n\r"
                    "         imc info host    <~!InfoHost~c> (server ip addy or dns name)\n\r"
                    "         imc info port    <~!InfoPort~c> (the muds port)\n\r"
                    "         imc info imail   <~!admins imc mail address~c>\n\r"
                    "         imc info email   <~!admins internet mail address~c>\n\r"
                    "         imc info www     <~!URL for webpage~c>\n\r"
                    "         imc info details <~!text~c>\n\r"
                    "         imc reload~!\n\r"),
                    ch);
        return;
    }
...
#ifdef CIRCLE
  imc_to_char(color_itom((char *)imc_getstats()), ch);
#else
  imc_to_char(color_itom(imc_getstats()), ch);
#endif
}
...
DEFINE_DO_FUN(do_rchannels)
{
    ...
    if (!arg[0])
    {
        ...
        for (i=0; i<numchannels; i++)
        {
            ...
            {
                sprintf(buf+strlen(buf),
                        "~c%-10s ~!: you are denied from this channel.\n\r",
                        imc_channels[i].name);
                found=TRUE;
            }
        ...
        if (IS_SET(IMC_DENY_FLAGS(ch), IMC_RTELL))
        {
            strcat(buf, "~cRTells     ~!: you cannot rtell.\n\r");
            found=TRUE;
        }
        else if (CAN(ch, IMC_RTELL, IMC_LEVEL_RTELL))
        {
            sprintf(buf+strlen(buf), "~c%-10s ~!: %s~!\n\r", "RTell",
                    IS_SET(IMC_DEAF_FLAGS(ch), IMC_RTELL) ? "~ROFF" : "~GON");
            found=TRUE;
        }

        if (IS_SET(IMC_DENY_FLAGS(ch), IMC_RBEEP))
        {
            strcat(buf, "~cRBeeps     ~!: you cannot rbeep.\n\r");
            found=TRUE;
        }
        else if (CAN(ch, IMC_RBEEP, IMC_LEVEL_RBEEP))
        {
            sprintf(buf+strlen(buf), "~c%-10s ~!: %s~!\n\r", "RBeep",
                    IS_SET(IMC_DEAF_FLAGS(ch), IMC_RBEEP) ? "~ROFF" : "~GON");
            found=TRUE;
        }

        if (IS_SET(IMC_DEAF_FLAGS(ch), IMC_RINVIS))
            sprintf(buf+strlen(buf), "~cYou are not visible to others over IMC.~!\n\r");
        else
            sprintf( buf+strlen(buf), "~cYou are currently visible to others over IMC.~!\n\r" );
			  ...
			  const char *argument, int emote)
{
  ...
  str= color_itom( emote ? imc_channels[chan].emotestr : imc_channels[chan].chatstr );
...
int imc_strlen_color( const char *argument )
{
    const char  *str;
    int    i, length;
    ...
    {
        if( str[i] != IMC_COLORCHAR && str[i] != IMC_COLORCHAR2 )
            ++length;
        if( str[i] == IMC_COLORCHAR || str[i] == IMC_COLORCHAR2 )
        {
            if( (str[i] == IMC_COLORCHAR && str[i+1] == IMC_COLORCHAR )
                || (str[i] == IMC_COLORCHAR2 && str[i+1] == IMC_COLORCHAR2 ))
                length += 2;
            else
                --length;
        }
...
char *SetFill( char *string )
{
    ...
    amount = 78-imc_strlen_color( string ); /* Determine amount to put in
                                      * front of line
                                      */
    if( amount < 1 )
        amount = 1;
...
void process_rwho(const imc_char_data *from, const char *argument)
{
    ...
    sprintf(output, "\n\r~WRWHO for %s ~Y[~W%d player%s found~Y]~!\n\r",
            imc_name, count, (count > 1) ? "s" : "" );

    sprintf( output, "%s", color_itom( output ) );
    ...
    /* Elmseeker 1/30/00 */
#ifdef ACK
#ifndef I_WANT_RWHO_LOGGING
    sprintf( buf, "RWHOed by %s ", from->name);
...
void imc_recv_who(const imc_char_data *from, const char *type)
{
...
#ifdef I_WANT_RWHO_LOGGING
    {
        char buf[200];
        sprintf( buf, "%s request from %s.\n\r", arg, from->name );
        imc_log( buf ); /* Kratas@Eternal 10/16/99 */
    }
#endif
    ...
    else if (!str_cmp(arg, "istats"))
    {
        ...
        strcpy(output,
               ...
               "~chelp       ~W- ~!this list\n\r"
               "~cwho        ~W- ~!who listing\n\r"
               "~cinfo       ~W- ~!mud information\n\r"
               "~clist       ~W- ~!active IMC connections\n\r"
               "~cdirect     ~W- ~!direct IMC connections\n\r"
               "~cconfig     ~W- ~!local IMC configuration\n\r"
               "~cistats     ~W- ~!network traffic statistics\n\r"
               /* uncomment if you support rfinger */
               /* "~cfinger xxx ~W- ~!finger player xxx\n\r" */
              );
    else
...
void imc_recv_whoreply(const char *to, const char *text, int sequence, int inlen)
{
    DESCRIPTOR_DATA *d;
    CHAR_DATA *victim;
    /* compressed who - shogar - 1/24/2000 */
    lzo_byte  compoutstr[IMC_DATA_LENGTH + IMC_DATA_LENGTH / 64 + 16 + 4];
    lzo_uint new_len=0;
    int compressed = FALSE;

    /*
     * compressed who - shogar - 1/24/2000
     * Moved to out herebecause pointless to decompress all the time-- Kratas
     */
    if( compressit )
        compressed = lzo_decompress( text, inlen, compoutstr, &new_len, NULL );
    ...
    for (d=descriptor_list; d; d=d->next)
    {
        ...
        {
            if( compressit && compressed ) /* compressed who - shogar - 1/24/2000 */
                imc_to_pager(color_itom(compoutstr), victim);
            else
...
/*Elmseeker 1/30/00 */
#ifdef ACK
  sprintf( buf1, "RWHOIS by %s for %s", from->name, to );
  monitor_chan( buf1, MONITOR_CONNECT );
#endif
}
		   ...
		   const char *argument, int isreply)
{
  ...
  if (!strcmp(to, "chat")) 
  {
	imc_recv_chat_command(from->name, to, argument, isreply);
	return;
  }
#endif
...
void imc_recv_beep(const imc_char_data *from, const char *to)
{
  ...
  if (victim)
  {
    ...
    sprintf(buf, color_itom(RBEEP_FORMAT_2), from->name);
    imc_to_char(buf, victim);
  }
  else
  {
