...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( !IS_NPC(victim)
    &&   xIS_SET(victim->act, PLR_AFK) )	strcat( buf, "[AWAY] ");        
...
/*        strcat( buf, victim->pcdata->title ); */
    ...
    case POS_SITTING:
        ...
        if(victim->fur_pos > FURNITURE_NONE) {
        	...
        	sprintf(buf2, " %s ", pos_string(victim));
        	strcat(buf, buf2);
        	sprintf(buf2, "%s %s", fur_pos_string(victim->fur_pos), 
        		victim->on->short_descr);
        	
        	if(victim->on == ch->on) {
        		strcat(buf, "next to you ");
        		strcat(buf, buf2);
        		strcat(buf, ".");
        	} else {
        		strcat(buf, buf2);
    ...
    /* eaten by Prage, see case POS_SLEEPING*/
    /*
    if (victim->fur_pos > FURNITURE_NONE)
    {
        if (victim->on)
...
//  Ranks by number of matches between two whole words. Coded for Similar Helpfiles
//  by Senir.
sh_int str_similarity( const char *astr, const char *bstr )
{
   sh_int matches=0;
...
//  Coded for Similar Helpfiles by Senir.
sh_int str_prefix_level( const char *astr, const char *bstr )
{
   sh_int matches=0;
...
// Main function of Similar Helpfiles by Senir. It loops through all of the
...
void do_help( CHAR_DATA *ch, char *argument )
{
    ...
    set_pager_color(AT_WHITE,ch); /* fix the help pager color problem shogar */
...
/* start from last to first to get it in the proper order */
	...
	||   wch->level > iLevelUpper
     /*	|| ( !CAN_PKILL( wch ) ) */    /* If ya want only level 10 players and up to show, uncomment the line to the left */ 
	...
	{
	default: break;
	case MAX_LEVEL -  0: class = "&ROwner";	          break;
	case MAX_LEVEL -  1: class = "&RHead Imp.";	  break;
	case MAX_LEVEL -  2: class = "&RImplementor";	  break;
	case MAX_LEVEL -  3: class = "&RAsst. Imp.";	  break;
	case MAX_LEVEL -  4: class = "&RAdmin.";  	  break;
	case MAX_LEVEL -  5: class = "&cCoder";		  break;
	case MAX_LEVEL -  6: class = "&cAdmin Rep.";	  break;
	case MAX_LEVEL -  7: class = "&cMaster Bldr";	  break;
	case MAX_LEVEL -  8: class = "&cGreater Bldr";	  break;
	case MAX_LEVEL -  9: class = "&cBuilder";	  break;
	case MAX_LEVEL - 10: class = "&cLesser Bldr";	  break;
	case MAX_LEVEL - 11: class = "&cNew Builder";	  break;
	case MAX_LEVEL - 12: class = "&cGeneral Staff";	  break;
	case MAX_LEVEL - 13: class = "&BImm Family";	  break;
	case MAX_LEVEL - 14: class = "&BMUD Guest"; break;
	case MAX_LEVEL - 15: class = "&BAmbassador";  	  break;
	case MAX_LEVEL - 16: class = "&YDemi-God";	  break;
	}
	...
	if ( wch->pcdata->council )
	{
        ...
        if ( wch->pcdata->council2 )
        {
          ...
          {            
             if (!str_cmp (wch->name, wch->pcdata->council2->head)
                 || !str_cmp ( wch->name, wch->pcdata->council2->head2) )
...
*/
	...
	for ( sn = 0; sn < top_sn; sn++ )
	{
		...
		if ( col % 3 != 0 )
		    send_to_pager( "\n\r", ch );
	      set_pager_color( AT_MAGIC, ch );
	      send_to_pager_color( " &c---------------------------------[&W Practice &c]---------------------------------&w\n\r", ch);
		col = 0;
	    }
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_RACETALK )?
        ...
        ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_YELL )    ?
				                " &G+YELL"	          :
						" &g-yell" );
	ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_GRATZ )   ?
			                        " &G+GRATZ"         : 
                                                " &g-gratz" );
	ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_TECH )   ?
			                        " &G+TECH"         : 
                                                " &g-tech" );
	...
	&&   !str_cmp( ch->pcdata->council->name, "Newbie Council" ) ) )
          ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_NEWBIE) ?
    				 	        " &G+NEWBIE"	     	  :
						" &g-newbie" );
        if ( IS_IMMORTAL(ch) || ( ch->pcdata->council2
	&&   !str_cmp( ch->pcdata->council2->name, "Newbie Council" ) ) )
            ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_NEWBIE) ?
                                                " &G+NEWBIE" :
                                                " &g-newbie" );   
        ...
        if ( !IS_NPC( ch ) && ch->pcdata->council2)
          ch_printf_color( ch, "%s",   !IS_SET( ch->deaf, CHANNEL_COUNCIL2)?
                                                " &G+COUNCIL2" :
                                                " &g-council2" );   
	/* Immortal channels */
        if ( IS_IMMORTAL( ch ) )
	...
	else if ( !str_cmp( arg+1, "staff"    ) ) bit = CHANNEL_HIGHGOD;
	...
	else if ( !str_cmp( arg+1, "tech"     ) ) bit = CHANNEL_TECH;
	...
	if (( fClear ) && ( ClearAll ))
	{
	    ...
	    REMOVE_BIT (ch->deaf, CHANNEL_TECH);
    ...
    if ( arg[0] == '\0' )
    {
      ...
      ch_printf( ch, "%-12s   %-12s   %-12s   %-12s\n\r           %-12s   %-12s   %-12s   %-12s\n\r           %-12s   %-12s   %-12s",
        ...
        xIS_SET(ch->act, PLR_RIP )                  	? "[+] RIP"
                                                    	: "[-] rip",
		xIS_SET(ch->act, PLR_NOMENU )					? "[+] NOMENU"
														: "[-] nomenu",
...
/*	else if ( !str_prefix( arg+1, "flee"     ) ) bit = PLR_FLEE; */
	...
	else if ( !str_prefix( arg+1, "nomenu"   ) ) bit = PLR_NOMENU; /* Scion */
	...
	if (bit)
	  ...
	  else if ( !str_prefix( arg+1, "immproof" ) ) bit = PCFLAG_IMMPROOF;
...
{ 
    ...
    char *header_string2 = "-------------+-----------------"
                                    "---------------------+----"
                                    "---------+-----------\n\r";
    char *print_string = "&c%-12s&w | &C%-36s&w | &Y%4d - %-4d&w | &R%3d - "
                                    "%-3d&w \n\r";
   ...
   if ( arg1 && arg1[0] != '\0' && !is_number(arg1))
   {
        for ( classnum = 0; classnum < MAX_CLASS; classnum++ )
		{
			if ( class_table[classnum] && class_table[classnum]->who_name &&
				class_table[classnum]->who_name[0] != '\0' 
				&& str_cmp(class_table[classnum]->who_name,"unused"))
			{
		...
		sprintf( buf, "&c                  %s&w\n\r", class_table[classnum]->who_name );
		pager_printf( ch, buf ); 
   }        
   else
   {
   ...
   for ( i = lowlev; i <= hilev; i++)
   {
	   ...
	   for ( sn = 0; sn < top_sn; sn++ )
	   {
		   ...
		   pager_printf(ch, "&c%7s&w: %20.20s \t &cCurrent&w: %-3d &cMax&w: %-3d  &cMinPos&w: %s \n\r",
			   ...
			   skill_table[sn]->skill_adept[classnum],
			   buf );
...
void do_whois( CHAR_DATA *ch, char *argument)
{
    ...
    if ( pclan->clan_type == CLAN_ORDER ) {
      if ( !str_cmp( victim->name, pclan->deity ) )
        ...
        if ( !str_cmp( victim->name, pclan->leader ) )
          ...
          if ( !str_cmp( victim->name, pclan->number1 ) )
            send_to_pager( "&P Number One of the Order of ", ch);
          else
            if ( !str_cmp( victim->name, pclan->number2 ) )
  ...
  if(victim->pcdata->homepage && victim->pcdata->homepage[0] != '\0')
    pager_printf(ch, "&O %s homepage is: %s\n\r", 
        ...
        victim->sex == SEX_FEMALE ? "&pHer&O" : "&GIts&O",
	...
	if (xIS_SET(victim->act, PLR_MARRIED)) {
		pager_printf(ch, "&g %s is married to %s.",
                             ...
                             victim->sex == SEX_FEMALE ? "&pShe&g" : "&GIt&g",
                            victim->pcdata->wedto );
	}
  ...
  {  
	...
	victim->name, 
	(victim->pcdata->release_date == 0) ? "&Gnot&O " : "");
