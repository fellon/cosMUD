...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; sn < top_sn; sn++ )
	{
		...
		if ( ch->level < SKILL_TABLE_LEVEL(ch,sn)
	      || (!IS_IMMORTAL(ch) && SKILL_TABLE_LEVEL(ch,sn) == 0) )
 	        continue;
	...
	if ( can_prac 
          && ( ( sn == -1 )
       	       || ( !IS_NPC(ch)
	            &&  ch->level < SKILL_TABLE_LEVEL(ch,sn) ) 
             ) 
           )
	{
