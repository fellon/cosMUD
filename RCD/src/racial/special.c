...
bool dragon( CHAR_DATA *ch, char *spell_name )
{
    ...
    { /* ok , it checked it, but let me know too - shogar */
        bug("dragon spec: %s's %s not a spell",
		ch->name,spell_name);
	return FALSE;
    }
...
bool spec_breath_gas( CHAR_DATA *ch )
{
    ...
    if ( ( sn = skill_lookup( "gasbreath" ) ) < 0 )
	return FALSE;
...
{
    return dragon( ch, "lightningbreath" );
}
