...
void do_skin( CHAR_DATA *ch, char *argument)
{
   ...
   /* act( AT_MAGIC, "The skinless corpse disintigrates before your eyes!", ch, corpse, NULL, TO_CHAR);
    act( AT_MAGIC, "The skinless corpse disintigrates before your eyes!", ch, corpse, NULL, TO_ROOM);*/
...
void do_sset( CHAR_DATA *ch, char *argument )
{
	...
	if ( get_trust(ch) > LEVEL_GREATER )
	{
	  ...
	  send_to_char( "or:	sset copy <from class#> <to class#>\n\r",ch );
...

	{
		...
		if (arg3[0]=='\0' || arg2[0]=='\0') {
			...
			send_to_char("Syntax: sset copy <from class#> <to class#>\r\n",ch);
		...
		for (index=0; index < top_sn; index++) {
			...
			skill_table[index]->skill_level[class2] = (skill_table[index]->skill_level[class1]<=MAX_LEVEL ? skill_table[index]->skill_level[class1] : 101);

			skill_table[index]->skill_adept[class2] = (skill_table[index]->skill_adept[class1]<=MAX_LEVEL ? skill_table[index]->skill_adept[class1] : 101);
...
void learn_from_success( CHAR_DATA *ch, int sn )
{
	...
	gain /= 2; /* Twice the levels, half the gain -- Scion */
...
*/
	...
	if ((ch->level > victim->level + 10) && !IS_NPC(victim)) {

		send_to_char( "That would be far too easy.  Try something more difficult!\r\n",ch);
	...
	if ((ch->level < victim->level - 10) && !IS_NPC(victim)) {
		...
		send_to_char( "That seems impossible!  Maybe you should try someone more in your league?\r\n",ch);
	...
	if (IS_IMMORTAL(victim)) {
		...
		send_to_char("You really shouldn\'t steal from an immortal...\r\n",ch);
	...
	if (IS_IMMORTAL(ch)) {
		...
		send_to_char("You are immortal!  Why bother stealing that when you can create it from thin air?\r\n",ch);
...
void do_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_kick))
    {
	send_to_char(
...
void do_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_punch))
    {
	send_to_char(
	    "You better leave the martial arts to fighters.\n\r", ch );
	return;
...
void do_bite( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_bite))
    {
	send_to_char(
...
void do_claw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_claw))
    {
	send_to_char(
	    "That isn't quite one of your natural skills.\n\r", ch );
	return;
...
void do_sting( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_sting))
    {
	send_to_char(
	    "That isn't quite one of your natural skills.\n\r", ch );
	return;
...
void do_tail( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_tail))
    {
	send_to_char(
	    "That isn't quite one of your natural skills.\n\r", ch );
	return;
...
void do_bash( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_bash))
    {
	send_to_char(
	    "You better leave the martial arts to fighters.\n\r", ch );
	return;
...
void do_stun( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_stun))
    {
	send_to_char(
	    "You better leave the martial arts to fighters.\n\r", ch );
	return;
...
void do_disarm( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_disarm))
    {
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch)) {
        if (IS_AFFECTED(ch, AFF_CHARM) && ch->master && !IS_NPC(ch->master)) {
          location = get_room_index( ch->master->pcdata->memorize[0] );
        } else {
          return;
        }
    } else {
   ...
   /* &&   IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) */ )
	...
	if ( number_bits( 1 ) == 0 || ( !IS_NPC( opponent ) && number_bits( 3 ) > 1 ) )
	{
	    ...
	    if ( ch->desc )
	      lose /= 2;
	    /* gain_exp( ch, 0 - lose );
	    ch_printf( ch, "You failed!  You lose %d experience.\n\r", lose );
              -- exp loss removed at Keolah's request -- Scion */
	    return;
	}


	ch_printf(ch, "You cannot recall from combat!\r\n");
...
void do_mount( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_mount))
    {
	send_to_char(
...
void do_scribe( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_scribe))
    {
	send_to_char( "A skill such as this requires more magical ability than what you have.\n\r", ch );
	return;
    }
...
void do_brew( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch)
    &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_brew))
    {
        send_to_char( "A skill such as this requires more magical ability than what you have.\n\r", ch );
	return;
    }
...
bool check_illegal_psteal( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if (!IS_NPC (victim) && !IS_NPC(ch))
    {
	if (/* ( !IS_SET( victim->pcdata->flags, PCFLAG_DEADLY ) || */ch->level - victim->level > 10
    /*	|| !IS_SET( ch->pcdata->flags, PCFLAG_DEADLY ) ) */
	&& ch != victim )
	{
...
/* gsn_kick was used in this if statement , I fix - shogar */
  if ( !IS_NPC(ch) && !IS_IMMORTAL(ch)
  &&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_slice))
  {
