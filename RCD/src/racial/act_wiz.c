...
void do_showrace( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "\n\r", ch);
	send_to_char( "Racial Skills:\n\r\n\r", ch);

    for ( index = 0; index <= MAX_LEVEL; index++ )
    {
        set_pager_color( AT_PLAIN, ch );
        for ( skill = gsn_first_racial; skill < gsn_top_sn; skill++ )
        {
            if ( skill_table[skill]->race_level[ra] == index 
              && skill_table[skill]->type == SKILL_RACIAL )
            {
                if ( index >= LEVEL_IMMORTAL && !immortal )
                {
                   pager_printf( ch, "\n\r**Immortal Only**\n\r\n\r");
                   immortal = TRUE;
                }
	        pager_printf( ch, "%-19s Adept: %3d Level: %3d     ",
		skill_table[skill]->name, skill_table[skill]->race_adept[ra], index );
		send_to_pager( "\n\r", ch );
	    }
	}
    }
