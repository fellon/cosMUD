...
sh_int			gsn_fourth_cast;
sh_int			gsn_fifth_cast;
sh_int			gsn_sixth_cast;
sh_int			gsn_seventh_cast;
sh_int			gsn_eighth_cast;
...
void boot_db( bool fCopyOver )
{
    ...
    sysdata.staff_level			= LEVEL_DEMI;
	...
	if ( !gsn_first_racial && skill_table[x]->type == SKILL_RACIAL )
	    gsn_first_racial = x;
...
void load_mobiles( AREA_DATA *tarea, FILE *fp )
{
	...
	*/
        ...
        {
		bug("Mob %d was set for level %d ... Resetting to level %d",
			vnum,pMobIndex->level,LEVEL_AVATAR);
...
/* NEVER RESET if the total number of objects is less than the number of areas * 60  -- Scion */
	if ( (pArea->nplayer == 0 || pArea->age >= reset_age) &&
		( (physicalobjects < (top_area*60)) || (nummobsloaded < (top_area*60)) ) )
	{
	    ROOM_INDEX_DATA *pRoomIndex;
...
/*  towizfile( " Masters of the Realms of Despair!" );*/
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next ) {
	if ( wiz->level < ilevel ) {
		...
		switch(ilevel)
		{
	case MAX_LEVEL -  0: towizfile( "   &ROmnipotent &w(Level 116) &ROwner&W" ); break;
	case MAX_LEVEL -  1: towizfile( "   &RSupreme Entity &w(Level 115) &RHead Implementors&W" ); break;
	case MAX_LEVEL -  2: towizfile( "   &RInfinite &w(Level 114) &RImplementors&W" ); break;
	case MAX_LEVEL -  3: towizfile( "   &REternal &w(Level 113) &RAssistant Implementors&W" ); break;
	case MAX_LEVEL -  4: towizfile( "   &RAncient &w(Level 112) &RAdministrators&W" ); break;
	case MAX_LEVEL -  5: towizfile( "   &cExalted &w(Level 111) &cCoders&W" ); break;
	case MAX_LEVEL -  6: towizfile( "   &cAscendant &w(Level 110) &cRepresentatives&W" ); break;
	case MAX_LEVEL -  7: towizfile( "   &cMaster Gods &w(Level 109) &cMaster Builders&W" ); break;
	case MAX_LEVEL -  8: towizfile( "   &cGreater Gods &w(Level 108) &cGreater Builders&W" ); break;
	case MAX_LEVEL -  9: towizfile( "   &cGods &w(Level 107) &cBuilders&W" ); break;
	case MAX_LEVEL - 10: towizfile( "   &cLesser Gods &w(Level 106) &cLesser Builders&W" ); break;
	case MAX_LEVEL - 11: towizfile( "   &cNew Gods &w(Level 105) &cNew Builders&W" ); break;
	case MAX_LEVEL - 12: towizfile( "   &cSaviors &w(Level 104) &cGeneral Staff&W" ); break;
	case MAX_LEVEL - 13: towizfile( "   &BCreators &w(Level 103) &BImmortal Family&W" ); break;
	case MAX_LEVEL - 14: towizfile( "   &BAcolytes &w(Level 102) &BInter-MUD Guest&W" ); break;
	case MAX_LEVEL - 15: towizfile( "   &BNeophytes &w(Level 101) &BAmbassador&W" ); break;
	case MAX_LEVEL - 16: towizfile( "   &BRetired&W" ); break;
	case MAX_LEVEL - 17: towizfile( "   &BGuests&W" ); break;
	default:	     towizfile( "   &BServants&W" ); break;
		}
	}
...
void make_webwiz( )
{
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
	case MAX_LEVEL -  0: towebwiz( "<P>&ROmnipotent &w(Level 116) &ROwner<BR>&W" ); break;
	case MAX_LEVEL -  1: towebwiz( "<P>&RSupreme Entity &w(Level 115) &RHead Implementors<BR>&W" ); break;
	case MAX_LEVEL -  2: towebwiz( "<P>&RInfinite &w(Level 114) &RImplementors<BR>&W" ); break;
	case MAX_LEVEL -  3: towebwiz( "<P>&REternal &w(Level 113) &RAssistant Implementors<BR>&W" ); break;
	case MAX_LEVEL -  4: towebwiz( "<P>&RAncient &w(Level 112) &RAdministrators<BR>&W" ); break;
	case MAX_LEVEL -  5: towebwiz( "<P>&cExalted &w(Level 111) &cCoders<BR>&W" ); break;
	case MAX_LEVEL -  6: towebwiz( "<P>&cAscendant &w(Level 110) &cRepresentatives<BR>&W" ); break;
	case MAX_LEVEL -  7: towebwiz( "<P>&cMaster Gods &w(Level 109) &cMaster Builders<BR>&W" ); break;
	case MAX_LEVEL -  8: towebwiz( "<P>&cGreater Gods &w(Level 108) &cGreater Builders<BR>&W" ); break;
	case MAX_LEVEL -  9: towebwiz( "<P>&cGods &w(Level 107) &cBuilders<BR>&W" ); break;
	case MAX_LEVEL - 10: towebwiz( "<P>&cLesser Gods &w(Level 106) &cLesser Builders<BR>&W" ); break;
	case MAX_LEVEL - 11: towebwiz( "<P>&cNew Gods &w(Level 105) &cNew Builders<BR>&W" ); break;
	case MAX_LEVEL - 12: towebwiz( "<P>&cSaviors &w(Level 104) &cGeneral Staff<BR>&W" ); break;
	case MAX_LEVEL - 13: towebwiz( "<P>&BCreators &w(Level 103) &BImmortal Family<BR>&W" ); break;
	case MAX_LEVEL - 14: towebwiz( "<P>&BAcolytes &w(Level 102) &BInter-MUD Guest<BR>&W" ); break;
	case MAX_LEVEL - 15: towebwiz( "<P>&BNeophytes &w(Level 101) &BAmbassador<BR>&W" ); break;
	case MAX_LEVEL - 16: towebwiz( "<P>&BRetired<BR>&W" ); break;
	case MAX_LEVEL - 17: towebwiz( "<P>&BGuests<BR>&W" ); break;
...
void fread_sysdata( SYSTEM_DATA *sys, FILE *fp )
{
	...
	case 'I':
	    KEY( "IdentTries",	   sys->ident_retries,	  fread_number( fp ) );
            KEY( "IMCMailLevel",   sys->imc_mail_level,   fread_number( fp ) );
...
/* Scion arenacode */
	...
	case 'M':
	    ...
	    KEY( "Staff",	   sys->staff_level,	   fread_number( fp ) );
	    break;
