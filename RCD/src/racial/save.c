...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload )
{
	...
	case 'A':
	    ...
	    KEY( "ArenaDeaths", ch->arena_deaths,	fread_number( fp ) );
	...
	case 'L':
		...
		if ( isalpha(ch->pcdata->lastname[0])
		||   isdigit(ch->pcdata->lastname[0]) )
		{
	...
	case 'R':
		...
		else
		{
		  ...
		  if ( file_ver < 3 )
		    ...
		    sn = bsearch_skill_exact( fread_word( fp ), gsn_first_racial, gsn_top_sn-1 );
		  if ( sn < 0 )
		    bug( "Fread_char: unknown racial.", 0 );
		  else
		  {
		    ...
		    if ( ch->level < LEVEL_IMMORTAL )
		    { 
		      if ( SKILL_TABLE_LEVEL(ch,sn) >= LEVEL_IMMORTAL )
		      {
	...
	case 'E':
		...
		if ( isalpha(ch->pcdata->extraname[0])
		||   isdigit(ch->pcdata->extraname[0]) )
		{
	...
	case 'T':
		...
		else
		{
		  ...
		  sn = bsearch_skill_exact( fread_word( fp ), gsn_first_tongue, gsn_first_racial-1 );
		  if ( sn < 0 )
...
/*	case 'M':
		...
		KEY("Material", obj->material, (material_lookup(fread_number(fp)) ? material_lookup(fread_number(fp)) : 0));
		break;  */
	case 'N':
	...
	if (objfile) {
		for ( obj = first_object; obj; obj = obj->next ) {
			...
			if (IS_OBJ_STAT( obj, ITEM_PROTOTYPE )
				...
				|| (obj->in_room && 
					(IS_SET(obj->in_room->room_flags, ROOM_DONATION) || IS_SET(obj->in_room->room_flags, ROOM_CLANSTOREROOM))))
				continue;
			else {
