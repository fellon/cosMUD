...
int skill_lookup( const char *name )
{
    ...
    if ( (sn=bsearch_skill_exact(name, gsn_first_spell, gsn_first_skill-1)) == -1 )
      if ( (sn=bsearch_skill_exact(name, gsn_first_skill, gsn_first_weapon-1)) == -1 )
	if ( (sn=bsearch_skill_exact(name, gsn_first_weapon, gsn_first_tongue-1)) == -1 )
	  if ( (sn=bsearch_skill_exact(name, gsn_first_tongue, gsn_first_racial-1)) == -1 )
	    if ( (sn=bsearch_skill_exact(name, gsn_first_racial, gsn_top_sn-1)) == -1 )
	      if ( (sn=bsearch_skill_prefix(name, gsn_first_spell, gsn_first_skill-1)) == -1 )
	        if ( (sn=bsearch_skill_prefix(name, gsn_first_skill, gsn_first_weapon-1)) == -1 )
		  if ( (sn=bsearch_skill_prefix(name, gsn_first_weapon, gsn_first_tongue-1)) == -1 )
		    if ( (sn=bsearch_skill_prefix(name, gsn_first_tongue, gsn_first_racial-1)) == -1 )
		      if ( (sn=bsearch_skill_prefix(name, gsn_first_racial, gsn_top_sn-1)) == -1
		         && gsn_top_sn < top_sn )
		      {
		         for ( sn = gsn_top_sn; sn < top_sn; sn++ )
		         {
...
{
    if ( IS_NPC(ch) || !know )
	return bsearch_skill( name, gsn_first_tongue, gsn_first_racial-1 );
    else
	return ch_bsearch_skill( ch, name, gsn_first_tongue, gsn_first_racial-1 );
}
...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "That is only available to members of a certain club.\n\r", ch);
	    return;
	}

	/* mana = IS_NPC(ch) ? 0 : UMAX(skill->min_mana,
	   100 / ( 2 + ch->level - skill->skill_level[ch->class] ) ); -- Scion */
...
/* 	mana = IS_NPC(ch) ? 0 : UMAX(skill->min_mana,
	...
	chance = (IS_NPC(ch) ? ch->level : (int)( ( LEARNED(ch, gsn_second_cast) + (ch->level / 10) )/1.5 ));
...
/* added these to prevent core dumps for new max level - shogar */
    59, 59, 60, 60, 61, 61, 62, 62, 63, 63,
    64, 64, 65, 65, 66, 66, 67, 67, 68, 68,
    69, 69, 70, 70, 71, 71, 72, 72, 73, 73,
    74, 74, 75, 75, 76, 76, 77, 77, 78, 78 /* up to 120 */
    };
    int dam;
...
/*    if ( !IS_NPC( victim ) && !IS_NPC( ch ) )
  ...
  -- What's wrong with a little charming between friends? -- Scion */
...
/* added to prevent core dumbs for new max level - shogar */
    38, 39, 39, 40, 40, 41, 41, 42, 42, 43,
    44, 44, 45, 45, 46, 46, 47, 47, 48, 48,
    49, 49, 50, 50, 51, 51, 52, 52, 53, 53,
    54, 54, 55, 55, 56, 56, 57, 57, 58, 58 /* up to 120 */
    };
    AFFECT_DATA af;
    int dam;
...
/* added to prevent core dumbs for new max level - shogar */
    ...
    107, 107, 108, 108, 109, 109, 110, 110, 111, 111 /* up to 120 */
    };
    int dam;
...
/* added these to prevent new max level core dump - shogar */
    170, 170, 172, 172, 174, 174, 176, 176, 178, 178,
    180, 180, 182, 182, 184, 184, 186, 186, 188, 188,
    200, 200, 201, 201, 202, 202, 203, 203, 204, 204, /* slow it down a bit */
    205, 205, 206, 206, 207, 207, 208, 208, 209, 209 /* up to 120 */
    };
    int dam;
...
ch_ret spell_gate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    char_to_room( victim=create_mobile( temp ),
	ch->in_room );
	...
	act( AT_MAGIC, "$n summons a vampire!", ch, NULL, NULL, TO_ROOM );
	act( AT_MAGIC, "You summon a vampire!", ch, NULL, NULL, TO_CHAR );
...
/* add more to prevent core dump for new max level - shogar */
    76, 76, 77, 77, 78, 78, 79, 79, 80, 80,
    81, 81, 82, 82, 83, 83, 84, 84, 85, 85,
    86, 86, 87, 87, 88, 88, 89, 89, 90, 90,
    91, 91, 92, 92, 93, 93, 94, 94, 95, 95 /* up to 120 */
    };
    int dam;
...
/* add more to prevent core dump for new max level - shogar */
    ...
    25, 25, 25, 25, 25, 26, 26, 26, 26, 26 /* up to 120 */
    };
    int dam;
...
/* added more to prevent new max level core dump - shogar */
    68, 68, 69, 69, 70, 70, 71, 71, 72, 72,
    73, 73, 74, 74, 75, 75, 76, 76, 77, 77,
    78, 78, 79, 79, 80, 80, 81, 81, 82, 82,
    83, 83, 84, 84, 85, 85, 86, 86, 87, 87 /* up to 120 */
    };
    int dam;
...
ch_ret spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	||   ( (IS_NPC(ch) == IS_NPC(victim)) && chance(ch, 10) ) )
	{
	    failed_casting( skill, ch, victim, NULL );
	    set_char_color( AT_MAGIC, victim );
	...
	act( AT_MAGIC, skill->hit_vict, ch, NULL, victim, TO_VICT );
		...
		act( AT_MAGIC, "You have been summoned by $n!", ch, NULL, victim, TO_VICT );
    if ( skill->hit_room && skill->hit_room[0] != '\0' )
	act( AT_MAGIC, skill->hit_room, victim, NULL, ch, TO_ROOM );
    else
		act( AT_MAGIC, "$n disappears in a flash of light!", victim, NULL, ch, TO_ROOM );

	char_from_room( victim );
    char_to_room( victim, ch->in_room );

    if ( skill->hit_dest && skill->hit_dest[0] != '\0' )
		act( AT_MAGIC, skill->hit_dest, victim, NULL, ch, TO_ROOM );
    else
		act( AT_MAGIC, "$n appears in a flash of light!", victim, NULL, ch, TO_ROOM );
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( skill->hit_vict && skill->hit_vict[0] != '\0' )
		...
		act( AT_MAGIC, "You have been teleported by $n!", ch, NULL, victim, TO_VICT );
    if ( skill->hit_room && skill->hit_room[0] != '\0' )
		act( AT_MAGIC, skill->hit_room, victim, NULL, ch, TO_ROOM );
    else
		act( AT_MAGIC, "$n slowly fades from view.", victim, NULL, ch, TO_ROOM );
	...
	sprintf( buf, "%s teleported %s to room %d.", ch->name,
					     victim->name,
					     victim->in_room->vnum );
	log_string_plus( buf, LOG_NORMAL, ch->level );
	to_channel( buf, CHANNEL_MONITOR, "Monitor", UMAX( LEVEL_IMMORTAL, ch->level ) );
	...
	act( AT_MAGIC, "$n slowly fades into view.", victim, NULL, ch, TO_ROOM );

	do_look( victim, "auto" );
    return rNONE;
}

ch_ret spell_teleport_superior( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    ROOM_INDEX_DATA *pRoomIndex;
    SKILLTYPE *skill = get_skilltype(sn);
	...
	pRoomIndex = get_room_index(ch->pcdata->memorize[i]);
...
{
	if (ch != (CHAR_DATA *)vo)
		act(AT_MAGIC, "You attempt to recall $N.", ch, NULL, (CHAR_DATA *)vo, TO_CHAR);
...
ch_ret spell_transport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( skill->hit_char && skill->hit_char[0] != '\0' )
		act( AT_MAGIC, skill->hit_char, ch, obj, NULL, TO_CHAR );
	else
		act( AT_MAGIC, "$p slowly dematerializes from your hands.", ch, obj, NULL, TO_CHAR);
	if ( skill->hit_room && skill->hit_room[0] != '\0' )
		act( AT_MAGIC, skill->hit_room, ch, obj, NULL, TO_ROOM );
	else
	...
	if ( skill->hit_char && skill->hit_dest[0] != '\0' )
		act( AT_MAGIC, skill->hit_char, victim, obj, NULL, TO_CHAR );
	else
		act( AT_MAGIC, "$p slowly materializes in your hands, sent by $N.", victim, obj, ch, TO_CHAR);
	if ( skill->hit_room && skill->hit_dest[0] != '\0' )
		act( AT_MAGIC, skill->hit_room, victim, obj, NULL, TO_ROOM );
	else
		act( AT_MAGIC, "$p slowly materializes in $n's hands.", victim, obj, NULL, TO_ROOM);
...
/* added more to prevent new max level core dump - shogar */
         172, 174, 178, 180, 182,       184, 186, 188, 190, 192,
         194, 196, 198, 200, 202,       204, 206, 208, 210, 212,
         214, 216, 218, 220, 222,       224, 226, 228, 230, 232,
         234, 236, 238, 240, 242,       246, 248, 250, 252, 254 /* up to 120 */
    };
    int dam;
...
/* added bad skill check - shogar */
    if(!skill || sn == -1)
    {
	bug("bad skill %d passed to spell_smaug",sn);
	return rNONE;
    }
