...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
	...
	if ( (to_room=generate_exit(in_room, &pexit)) == NULL )
        { /* if this returns NULL, it will crash room_is_private below
		so bail out. - added return rNONE - shogar */
	    send_to_char( "Alas, you cannot go that way.\n\r", ch );
	    return rNONE;
	}
	...
	/*int iClass;*/
	int move;
/* Pretty sure we don't need to check for clubs anymore now that we have
   the new dh. -- Narn
*/
/*
...
/*        if (  IS_SET( to_room->area->flags, AFLAG_NOPKILL )
	    ...
	    set_char_color( AT_MAGIC, ch );
            ...
            return rNONE;
        } This is effectively a no-mort zone with this uncommented since every player is "deadly" -- Scion */
...
void do_bashdoor( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC( ch )
	&&   ch->level < SKILL_TABLE_LEVEL(ch,gsn_bashdoor))
	{
	    send_to_char( "You're not big enough to bash doors!\n\r", ch );
	    return;
	}
...
void do_sit( CHAR_DATA *ch, char *argument )
{
	...
	if(arg[0])
	{
		...
		if (ch->weight >= ((obj->value[0] * 300) + number_range(-30, 30))) {
...
/* lets try this, the || will let the if continue, and the abs(xit->pull
   will crash the mud so.... add another set of parens - shogar
	if ( xtmp->pull && xtmp->to_room && (!xit || abs(xtmp->pull) > abs(xit->pull)) )

ok... split this up, one eval per line so gdb will tell me which one crashes
*/
	if ( xtmp->pull 
        && xtmp->to_room 
        && (!xit 
            || (abs(xtmp->pull) > abs(xit->pull))) )
	    xit = xtmp;
...
/* Sets a character's home room -- Scion
   Bugs and Format Fixed by Anonymous
   */
void do_memorize(CHAR_DATA *ch, char *argument)
{
	...
	switch (ch->substate)
		...
		{
			send_to_char_color("&C&wSyntax: memorize <slot number, 0 for nonmage>\n\r", ch);
			send_to_char_color("&C&w        memorize list\n\r", ch);
			return;
		}
		...
		{
			send_to_char_color("&C&wYour memorized locations:\r\n",ch);
			
			for (slot=0; slot < 8; slot++)
			{
				room = get_room_index(ch->pcdata->memorize[slot]);
				ch_printf_color( ch, "&C&w%d: %s\n\r", slot, (room ? room->name : "None"));
			}

			send_to_char_color("&C&wRecall uses location 0. All other locations only usable by mage.\n\r",ch);
			return;
		}

	        if ( IS_SET( ch->in_room->room_flags, ROOM_NO_ASTRAL ) )
                {
                        send_to_char_color( "&C&wIt would be useless to memorize this room.\n\r", ch );
                        return;
                }
		...
		if (!is_number(argument))
		{
			...
			return;
               ...
               slot = URANGE( 0, atoi(argument), 7);
               send_to_char_color( "&C&wYou begin to memorize your surroundings...\n\r", ch );
               act( AT_PLAIN, "$n looks around, trying to memorize $s surroundings...", ch, NULL, NULL, TO_ROOM);
               ch->tempnum = slot;
               add_timer( ch, TIMER_DO_FUN, 3, do_memorize, 1);
               return;

     case 1:

                ch->substate = SUB_NONE;
		if ( ch->pcdata->memorize[ch->tempnum] > 0)
			send_to_char_color( "&C&wReplacing previous occupant of memorize slot.\n\r", ch);
                ch->pcdata->memorize[ch->tempnum] = ch->in_room->vnum;
                act( AT_PLAIN, "$n has successfully memorized $s surroundings.", ch, NULL, NULL, TO_ROOM);
                send_to_char_color( "&C&wYou successfully memorized the room.\r\n",ch);
                return;

    case SUB_TIMER_DO_ABORT:

                act( AT_PLAIN, "$n cannot seem to concentrate on memorizing.",ch, NULL, NULL, TO_ROOM);
                send_to_char_color("&C&wYou just can't seem to concentrate...\n\r",ch);
                ch->substate = SUB_NONE;
                return;
    }
