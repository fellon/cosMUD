...
{
    return ((race_table[ch->race]->base_age) + 17 + ( ch->played + (current_time - ch->logon) ) / 7200);
}
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    {
	bug( "Equip_char: already equipped %s->%s Room %d.", obj->short_descr, ch->name, ch->in_room->vnum );
	bug( "Attempting to fix...");
	obj_from_char(obj);
	obj_to_char(obj, ch);
	do_wear(ch, obj->name);
	return;
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
    ...
    if( NOT_AUTHED( victim ) )
    {
      ...
      if( ch->pcdata->council && !str_cmp( ch->pcdata->council->name, "Newbie Council" ) )
        return TRUE;
      if( ch->pcdata->council2 && !str_cmp( ch->pcdata->council2->name, "Newbie Council" ) )
	return TRUE;
      return FALSE;
    }  
 ...
 * Updated 4/7/1999 by Scion to not depend on the name :P
 */
