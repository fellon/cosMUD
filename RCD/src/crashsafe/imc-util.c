/* $Id: imc-util.c,v 1.2 2000/02/03 13:56:32 gmoody Exp $ */
/*
...
{
  imc_reminfo *p,*ptmp,*plst,*pnxt;

  
  plst=NULL;
  for (p = imc_reminfo_list; p; p = pnxt)
  {
	pnxt=p->next;
	if(!pnxt)
		continue;
		
    	if (strcasecmp(p->name,pnxt->name)> 0)
	{
		if(!plst)
		{
			imc_reminfo_list = pnxt;
			ptmp=pnxt->next;
			pnxt->next=p;
			p->next=ptmp;
			plst=NULL;
			pnxt=imc_reminfo_list;
