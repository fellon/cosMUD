...
/*	    *pn = '\0';
	    if (is_name("all", arg_passed))
	    {
	        ch_printf(ch, "You cannot send IMC mail to ALL.\n\r");
	        return;
	    }
	    *pn = '@';*/
	    imc = TRUE;
...
/*	if (imc)
	{
	  ...
	  if (is_name("all", to))
	  {
	    send_to_char( "You may not post notes to ALL on the IMC board.\n\r", ch);
	    return;
	  }
	  if (np)
	    *np = '@';
	...
	for ( pnote = board->first_note; pnote; pnote = pnote->next )
	{
 		...
 		if ( take != 0 )
		{
		    ...
		    sprintf(keyword_buf, "note parchment paper mail %s", 
			pnote->to_list);
		    STRFREE(paper->name);
