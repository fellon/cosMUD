/* $Id: imc-interp.c,v 1.15 2000/02/12 13:16:24 gmoody Exp $ */
/*
...
/* LZO Compression Wrapper */
int lzo_compress( const lzo_byte *, lzo_uint, lzo_byte *, lzo_uint *, lzo_voidp, char * );

/* moved here  for 2.00 - shogar - 1/25/2000 */
ice_channel *next_refresh,*iced_channel_list; 
...
int mud_has_hub=0,compressit; /* for ping control - shogar */
...
#ifdef CRBS
void crbs_waffle_pseudos(const char *);
#endif
...
void imc_recv_ping(const char *from, int time_s, int time_u, const char *path)
{
  ...
  /* if only hubs can ping - or a mud acting as a hub ... then ...
     ...
     /* rev 9 */
/* good idea before hub switching - shogar 
     if(imc_getinfo(from) && !(imc_getinfo(from)->flag) & IMC_NEW))
...
void imc_recv(const imc_packet *p)
{
    ...
    /* who-reply: receive a compressed who response - shogar - 1/24/2000 */
    else if (!strcmp(p->type, "wHo-reply"))
    {
        compressit=1;
        imc_recv_whoreply(p->to,
                          imc_getkey(&p->data, "text", ""),
                          imc_getkeyi(&p->data, "sequence", -1),
                          imc_getkeyi( &p->data, "complen", -1 ));
        compressit=0;
    }
    /* who-reply: receive a who response */
    else if (!strcasecmp(p->type, "who-reply"))
        imc_recv_whoreply(p->to, imc_getkey(&p->data, "text", ""),
                          imc_getkeyi(&p->data, "sequence", -1), -1);

    /* who: receive a who request from a compression complient client - shogar*/
    /* 1/24/2000 */
    else if (!strcmp(p->type, "wHo"))
    {
        if (imc_isignored(p->from))
        {
            imc_sendignore(p->from);
    ...
    else if (!strcasecmp(p->type, "keepalive-request"))
    {
        ...
        imc_send_keepalive();
        /* reset next to begining, start notifying for 2.00 - shogar - 1/25/2000 */
        if(imc_is_router)
        {
            if(next_refresh) /* watch for streaming is-alives */
                imc_cancel_event(ev_iced_refresh,NULL);
    ...
    else if (!strcasecmp(p->type, "close-notify"))
    {
        ...
        if(imc_is_router)
            return;
#ifdef CRBS
	crbs_waffle_pseudos(imc_getkey(&p->data,"host","unknown"));
#endif
    ...
    /* These 4 added by me for the auto-hub-swap -- Scion 1/9/99 */
        ...
        /* Add a connection back to the hub */
        imc_autoconnect_reply_accept(p->from,
...
void imc_send_who(const imc_char_data *from, const char *to, const char *type)
{
  ...
  sprintf(out.to, "*@%s", to);
  strcpy(out.type, "wHo"); /* compress who - shogar - 1/24/2000 */
...
void imc_send_whoreply(const char *to, const char *data, int sequence)
{
    imc_packet out;
    int compressed;
    /* compress who - shogar 1/24/2000 */
    lzo_uint inlen, compoutlen;
    lzo_byte  compoutstr[IMC_DATA_LENGTH + IMC_DATA_LENGTH / 64 + 16 + 4];
    char outcoded[IMC_DATA_LENGTH];
    ...
    inlen = strlen( data );
    compressed = lzo_compress( data, inlen,compoutstr, &compoutlen, wrkmem, outcoded );

    if( compressit && compressed ) /* compress whos - shogar 1/24/2000 */
        strcpy(out.type, "wHo-reply");
    else
        strcpy(out.type, "who-reply");
    strcpy(out.from, "*");

    if (sequence!=-1) /* moved here for compress who - shogar - 1/24/2000 */
        imc_addkeyi(&out.data, "sequence", sequence);

    /* compress whos - shogar 1/24/2000 */
    if( compressit && compressed )
    {
        imc_addkeyi(&out.data, "complen", compoutlen);
        imc_addkey(&out.data, "text", outcoded);
    }
    else
    {
...
void imc_send_keepalive(void)
{
  ...
  imc_send(&out);
  imc_freedata(&out.data);
/* removed this , moved to keep_alive check - shogar - 1/25/2000 */
/*
  imc_add_event(ICED_REFRESH_TIME, ev_iced_refresh, NULL, 1);
*/
}
