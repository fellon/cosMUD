/* $Id: imc-config.c,v 1.19 2000/02/04 02:58:18 deaz Exp $ */
/*
...
int imc_readconfig(void)
{
    ...
    short infoport = 0; /* Local mud port -- kratas */
    ...
    while(1)
    {
        ...
        else if (!strcasecmp(word, "InfoPort"))
        {
            if( infoport > 0 )
                imc_logstring("Warning: duplicate 'InfoPort' lines");
            infoport = atoi( value );
        }
...
int imc_command(const char *argument)
{
    ...
    /* This used to be below the first 5 checks, moved up here now -- Kratas */
    if (!arg1[0] )
        return 0;
...
static int substr(const char *needle, const char *haystack)
{
  ...
  while (*h)
  {
    if(tolower(*h)==tolower(*needle))
      if (!strncasecmp(h, needle, len))
        return 1;
    h++;
  }
...
const char *imc_sockets(void)
{
  ...
  sprintf(buf, "~c%4s ~C%-9s ~B%-15s ~r%-11s ~R%-11s ~y%-3s ~Y%-3s\n\r",
  ...
  for (c=imc_connect_list; c; c=c->next)
  {
    ...
    sprintf(buf+strlen(buf),
	    "%4d %-9s %-15s ~r%5d/~R%-5d %5d/~r%-5d ~y%3d ~Y%3d\n\r",
	    c->desc,
...
const char *imc_list(int level)
{
    ...
    if (level==3 || level==6)
    {
        ...
        strcpy(buf, "~WActive muds on IMC:~!\n\r");
        if (imc_is_router)
            sprintf(buf + strlen(buf), "%-12s  %-8s  %-9s  %-20s  %-10s",
                    "Name", "Last alive", "Ping time", "IMC Version", "Route");
        else
            sprintf(buf + strlen(buf), "~c%-15.15s  ~C%-10.10s  ~B%-30.30s  ~G%-10.10s~!",
                    "Name", "Last alive", "IMC Version", "Route");

        for( p = imc_reminfo_list ; p ; p = p->next, count++ )
        {
            if (p->hide && level==3)
                continue;
            ...
            if (imc_is_router)
            {
                ...
                else
                {
                    sprintf(buf + strlen(buf), "\n\r~c%-13s  ~C%7ds  ~B%9s  ~G%-20s  ~!%-10s %s",
                            p->name, (int) (imc_now - p->alive), "~yunknown~!",
                            p->version, p->route ? p->route : "~Ybroadcast~!",
                            p->type ? "~Rexpired~!" : "");
                }
            }
            else
            {
                sprintf(buf + strlen(buf), "\n\r~c%-15.15s  ~C%9ds  ~B%-30.30s  ~G%-10.10s %s~!",
                        ...
                        p->route ? p->route : "~Ybroadcast", p->type ? "~Rexpired" : "");
            }
        }

        sprintf( buf + strlen( buf ), "\n\r%d muds on imc found.", count );
        imc_shrinksbuf( buf );
        return buf;
    }
...
const char *imc_getstats(void)
{
    ...
    if(imc_stats.txexpanded_bytes < 0L
       || imc_stats.txcompressed_bytes < 0L)
        imc_stats.txexpanded_bytes=imc_stats.txcompressed_bytes=0L;
    if(imc_stats.rxexpanded_bytes < 0L
       || imc_stats.rxcompressed_bytes < 0L)
        imc_stats.rxexpanded_bytes=imc_stats.rxcompressed_bytes=0L;
    ...
    sprintf(buf,
            ...
            imc_stats.rx_pkts,
            comma( imc_stats.rx_bytes ),
            imc_stats.rx_bytes /
            ((imc_now - imc_stats.start) ?
             (imc_now - imc_stats.start) : 1) );

    if(imc_stats.rxexpanded_bytes)
    {
        sprintf( buf+strlen( buf ),
                 "~cCompressed Recieved: ~C%ld/%ld (%ld%% savings)\n\r",
                 imc_stats.rxcompressed_bytes,
                 imc_stats.rxexpanded_bytes,
                 100-((imc_stats.rxcompressed_bytes*100/
                       imc_stats.rxexpanded_bytes)) );
    }
    else
    {
        sprintf(buf+strlen(buf),
                "~cCompressed Recieved: ~CNone (0%% savings)\n\r");
    }
    ...
    sprintf( buf+strlen( buf ),
             ...
             imc_stats.tx_pkts,
             comma( imc_stats.tx_bytes ),
             imc_stats.tx_bytes /
             ((imc_now - imc_stats.start) ?
              (imc_now - imc_stats.start) : 1) );

    if(imc_stats.txexpanded_bytes)
    {
        sprintf( buf + strlen( buf ),
                 "~cCompressed Sent    : ~C%ld/%ld (%ld%% savings)\n\r",
                 imc_stats.txcompressed_bytes,
                 imc_stats.txexpanded_bytes,
                 100-((imc_stats.txcompressed_bytes*100 /
                       imc_stats.txexpanded_bytes)) );
    }
    else
    {
        sprintf(buf+strlen(buf),
                "~cCompressed Sent    : ~CNone (0%% savings)\n\r");
    }
    ...
    sprintf( buf+ strlen( buf ),
             ...
             "~cTime between keepalives recieved:  ~C%ld seconds\n\r"
             "~cTime between pings              :  ~C%ld seconds\n\r"
             "~cTime between keepalives sent    :  ~C%ld seconds\n\r",
#else
             "~cSequence drops     : ~C%d\n\r"
             "~cLogging            : %s\n\r"
             "~cHubSwitch          : %s\n\r",
            ...
            imc_stats.sequence_drops,
            imc_log_on ? "~gYes" : "~rNo",
            imc_hubswitch ? "~gYes" : "~rNo" );
#endif
    sprintf(buf+strlen(buf),"~c%s\n\r",REVISION);
    sprintf(buf+strlen(buf),"~cLast IMC Boot: ~C%s~!\n\r",ctime(&imc_boot));
