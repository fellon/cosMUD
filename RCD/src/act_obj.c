...
/*		if ( IS_OBJ_STAT( container, ITEM_CLANCORPSE )
		...
		*** FULL CLAN CORPSE LOOTING ALLOWED. UNCOMMENT TO DISALLOW.
...
/*		    if ( !fGroup )
	...
	if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
	    ...
	    if ( IS_SET( sysdata.save_flags, SV_GET ) )
		save_char_obj( ch );
            else if ( IS_SET( sysdata.save_flags,  SV_DEATH ) && container->item_type == ITEM_CORPSE_PC )
	        save_char_obj( ch );
	}
	else
	{
...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace )
{
	...
	if ( obj == get_eq_char( ch, WEAR_FLOAT )) {

		act( AT_ACTION, "$n grabs $p from the air above $s head.", ch, obj, NULL, TO_ROOM );

		act( AT_ACTION, "You grab $p from the air above your head.", ch, obj, NULL, TO_CHAR );
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
    ...
    separate_obj( obj );
    if ( (get_trust( ch ) < obj->level ) && (obj->level >= 3) )
    {
	...
	act( AT_ACTION, "$n tries to use $p, but is too inexperienced.",
	    ch, obj, NULL, TO_ROOM );
	return;
    }  /* Fellon put them back. Becuase I like them. -- Fellon */
    ...
    {
	default:
	    bug( "wear_obj: uknown/unused item_wear bit %d. %s room %d", bit, ch->name, ch->in_room->vnum );
	    if ( fReplace )
		send_to_char( "You can't wear, wield, or hold that.\n\r", ch );
	    return;
...
*/
	...
	case ITEM_MISSILE_WIELD:
		...
		/* added to fix dual wield missile bug --Prage*/
		if(get_eq_char(ch, WEAR_MISSILE_WIELD)) {
			send_to_char("You can't wear two missiles at once.\n\r", ch);
			return;
		}
	...
	/* Floating items added by Scion */
	case ITEM_WEAR_FLOAT:
	    if (get_eq_char(ch, WEAR_FLOAT)) {
              send_to_char("You may only float one item at a time.\r\n",ch);
	      return;
	    }
	    if (!remove_obj(ch, WEAR_FLOAT, fReplace ) )
	        return;
            if (!oprog_use_trigger( ch, obj, NULL, NULL, NULL )) {
	      act(AT_ACTION, "You release $p, which floats gently above your head.", ch, obj, NULL, TO_CHAR);
	      act(AT_ACTION, "$n releases $p, which floats gently above $s head.", ch, obj, NULL, TO_ROOM);
	    }
...
/* finally... */
	...
	if (IS_OBJ_STAT(obj, ITEM_ARTIFACT)) {
		...
		send_to_char("You cannot bear to part with such a powerful artifact as that!\r\n",ch);
...
/*
 * Donate an object, by Tagith.  Proposed by Ntanel
 */
void do_donate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( donation = get_room_index( ROOM_VNUM_DONATE ) ) == NULL )
    {
	bug( "Donation room %d is non-existant!", ROOM_VNUM_DONATE );
	return;
    }

	if (IS_OBJ_STAT( obj, ITEM_ARTIFACT )) {

		send_to_char("You cannot bear to part with such a powerful artifact!\r\n", ch);
    ...
    separate_obj( obj );
    obj_from_char( obj );
    obj = obj_to_room( obj, donation );
    act( AT_MAGIC, "You donate $p.", ch, obj, NULL, TO_CHAR );

   /* objects copying, commented save routine out...
    if ( IS_SET( ch->in_room->room_flags, ROOM_CLANSTOREROOM ) )
    {
	CLAN_DATA *clan;
