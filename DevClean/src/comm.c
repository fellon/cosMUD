        ...
        if (sysdata.webtoggle==TRUE)
{
	...
	for ( d = first_descriptor; d; d = d_next )
	{
		...
		/* Reworked to avoid crashes -Marten */ 
		if ( d->connected == CON_PLAYING && d->character->pcdata && d->character->pcdata->alias_queue != NULL )
			...
			{	
				...
				if ( d->character ) 		 /* There is no more character after do_quit. */
				{
...
/*    int iSpec; */
	...
	if ( !d->character )
	{
	    ...
	    log_string( log_buf );
	    write_to_buffer( d, "Your playerfile is corrupt...Please notify fellon@sdf.lonestar.org.\n\r", 0 );
	    close_socket( d, FALSE );
	    return;
	}
	ch   = d->character;
...
/*	for ( vch = first_char; vch; vch = vch->next )
 ...
 Don't really need this.. just annoys me :)   -- Scion
	...
	if (!xIS_SET(ch->act, PLR_WIZINVIS))  /* when wizinvised entrance
info does not show */
	{
