                        ...
                        if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
                        {
                                ...
                                if (obj->value[1] <=1)
                                {
                                        if (ch->pcdata->condition[COND_BLOODTHIRST] < (ch->level + 10))
                                        {
                                                ...
                                                act( AT_BLOOD, "$n drinks the last drop of blood from the spill and desires more.", ch, NULL, NULL, TO_ROOM );
                                        }
                                        else
                                        {
                                                set_char_color( AT_BLOOD, ch );
                                                send_to_char( "You drink the last drop of blood from the spill and is satisfied.\n\r", ch);
                                                act( AT_BLOOD, "$n drinks the last drop of blood from the spill and is satisfied.", ch, NULL, NULL, TO_ROOM );
                                        }
                                }
                        }
...
void do_quaff( CHAR_DATA *ch, char *argument )
{
	...
	if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
	{
		...
		if ( ( liquid = obj->value[4] ) >= LIQ_MAX )
		{
	    bug( "Do_Quaff: bad liquid number %d.", liquid );
	    liquid = obj->value[4] = 0;
		}
