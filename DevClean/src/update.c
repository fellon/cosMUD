...
void gain_exp( CHAR_DATA *ch, int gain )
{
    ...
    /* Bonus for deadly lowbies - Who ever removed this was a lunkhead --Fellon */
    modgain = gain;
...
int hit_gain( CHAR_DATA *ch )
{
	...
	case POS_SLEEPING: gain += get_curr_con(ch) * 2.0; if(ch->on) gain += 5; break;
	case POS_RESTING:  gain += get_curr_con(ch) * 1.25;if(ch->on) gain += 2; break;
	}
...
int mana_gain( CHAR_DATA *ch )
{
	...
	switch ( ch->position )
	{
	case POS_SLEEPING: gain += get_curr_int(ch) * 3.25; if(ch->on) gain += 5;	break;
	case POS_RESTING:  gain += get_curr_int(ch) * 1.75; if(ch->on) gain += 2;	break;
	}
...
int move_gain( CHAR_DATA *ch )
{
	...
	case POS_INCAP:    return -1;
	case POS_STUNNED:  return 1;
	case POS_SLEEPING: gain += get_curr_dex(ch) * 4.5;if(ch->on) gain+=5;	break;
	case POS_RESTING:  gain += get_curr_dex(ch) * 2.5;if(ch->on) gain+=2;	break;
	}
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	if ((ch->desc) && ((ch->desc->connected == CON_PLAYING) && (ch->desc->idle > 1200 && !xIS_SET(ch->act, PLR_AFK))))
		do_afk(ch, "");
...
/* Clean up 'animated corpses' that are not charmed' - Scryn */
	...
	/* something else is wrong if I have to do this ... but until I can
	   track it down, this will have to do - shogar */
        if(! ch->in_room->area)
	{
		bug("Room %d for mob %s is not associated with an area?",
...
/*	if (number_range(1,5) < 4) {
	...
	/* You suck Scion. Thanks Rathkor.  -Marten */
	...
	&& ( door = number_bits( 5 ) ) <= 11 /* Was 9, 11 is secret+closed */
