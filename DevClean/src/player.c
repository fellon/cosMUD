...
void do_worth(CHAR_DATA *ch, char *argument)
{
    ...
    pager_printf(ch, "|Glory: %-4d |Weight: %-9d |Style: %-13s |Gold: %-14d |\n\r",
...
void do_score(CHAR_DATA * ch, char *argument)
{
	...
	pager_printf(ch, "&CDEX  : &W%2.2d&C(&w%2.2d&C)      Align: &W%-14.14s &C(&w%+4.4d&C)  Items: &W%5.5d   &C(&wmax %5.5d&C)\n\r",
...
/*  Fighting style support -haus
  ...
  Commented this out for now, not sure if we'll use them or not -- Scion
...
*/
     ...
     * Fighting style support -haus
     */
    pager_printf(ch, "&CCHA  : &W%2.2d&C(&w%2.2d&C)      Wimpy: &R%-5d \r\n",
	get_curr_cha(ch), ch->perm_cha, ch->wimpy);
 
	/* Commenting styles out for now -- Scion
        switch (ch->style) {
        case STYLE_EVASIVE:
                sprintf(buf, "evasive");
                break;
	...
	for (paf = ch->first_affect; paf; paf = paf->next)
	{
		...
		if ((++i % 3) == 0)
	     ...
	     {
		if (paf->modifier == 0)
		    pager_printf(ch, "&C[&W%-24.24s&C(&w%5d rds&C)]    ",
			sktmp->name,
			paf->duration);
		else
		if (paf->modifier > 999)
...
void do_newscore(CHAR_DATA * ch, char *argument)
{
    ...
    if (IS_VAMPIRE(ch))
	pager_printf_color(ch, "&CEXP  : &W%-9d       &CBlood: &R%-5d &Cof &r%5d   &CMKills:  &W%5d    &CAutoLoot(&W%c&C)\n\r",
...
void do_equipment( CHAR_DATA *ch, char *argument )
{
       ...
       bool found;
       int  cond; 
       char buf[MAX_STRING_LENGTH]; /* changed MSL to MAX_STRING_LENGTH --Prage */
       bool Display_all = TRUE; /*Set to false to only display used locations. */
        ...
        for ( obj = ch->first_carrying; obj; obj = obj->next_content )
           if ( obj->wear_loc == iWear )
           {
            ...
            { 
                ...
                {
                    send_to_char("&C<&R",ch);
                    sprintf(buf, "%3d%%", cond);  /*I prefer to have an acutal percentage rather than the ++++ thing :) */
                    send_to_char(buf, ch);
                    send_to_char("&C>&W  ",ch);
                }
                else
            ...
            found = TRUE;
            empty = FALSE;  /* If we have found something the this location isn't empty*/
       }
       if (Display_all == TRUE && empty == TRUE)
       {
                  set_char_color( AT_OBJECT, ch );
                  if( (!IS_NPC(ch)) && (ch->race>0) && (ch->race<MAX_RACE))
                send_to_char(race_table[ch->race]->where_name[iWear], ch);
...
void do_title( CHAR_DATA *ch, char *argument )
{
 ...
 /* This should be allowed to be blank, but if you don't like it uncomment here -- Scion
    if ( argument[0] == '\0' )
    {
...
void do_lastname( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level < 2 )
    {
	send_to_char( "Sorry... you must be at least level 2 to set your last name...\n\r", ch );
	return;
    }
    if ( IS_SET( ch->pcdata->flags, PCFLAG_NOTITLE ))
    {
...
/*  These should be allowed to be blank, but if you don't like it uncomment this -- Scion 
    if ( argument[0] == '\0' )
    {
...
void do_extraname( CHAR_DATA *ch, char *argument )
{
    ...
    {
	set_char_color( AT_IMMORT, ch );
        send_to_char( "The Gods prohibit you from changing your extra name.\n\r", ch );
        return;
    }
 
/* Should be able to make these blank, but if you don't like it, uncomment this. -- Scion
   if ( argument[0] == '\0' )
    {
	send_to_char( "Change your extra name to what?\n\r", ch );
	return;
    }
*/
