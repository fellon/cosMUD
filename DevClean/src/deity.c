...
void fread_deity( DEITY_DATA *deity, FILE *fp )
{
	...
	case 'D':
		...
		KEY( "Derecall",	deity->derecall,	fread_number( fp ) );
	    KEY( "Description",	deity->description,	fread_string( fp ) );
		KEY( "Detitle",		deity->detitle,		fread_string( fp ) );
		...
		KEY( "Domains",		deity->domains,		fread_string( fp ) );
	    break;
	...
	case 'E':
		...
		if (!deity->domains)
		  deity->domains		= STRALLOC( "" );
		if (!deity->detitle)
		  deity->detitle	  		= STRALLOC( "" );
	...
	case 'S':
	    ...
	    KEY( "Sdtrap",	deity->sdtrap,	    fread_number( fp ) );
	    ...
	    KEY( "Suplift", deity->suplift,		fread_number( fp ) );
...
void do_setdeity( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "scorpse savatar sdeityobj srecall suplift sdtrap\n\r\n\r", ch );
	send_to_char( "Vnums for Object, Avatar, and Recall\n\r", ch );
	send_to_char( "deityobj avatar derecall \n\r\n\r", ch );
    ...
    if ( !str_cmp( arg2, "domains" ) )
    {
	STRFREE( deity->domains );
	deity->domains = STRALLOC( argument );
	send_to_char( "Done.\n\r", ch );
	save_deity( deity );
	return;
    }

    if ( !str_cmp( arg2, "detitle" ) )
    {
	STRFREE( deity->detitle );
	deity->detitle = STRALLOC( argument );
	send_to_char( "Done.\n\r", ch );
	save_deity( deity );
	return;
    }
    ...
    if ( !str_cmp( arg2, "derecall" ) )
    {
        deity->derecall = atoi( argument );
        send_to_char( "Done.\n\r", ch );
        save_deity( deity );
        return;
    }
    ...
    if ( !str_cmp( arg2, "suplift" ) )
    {
	deity->suplift = atoi( argument );
	send_to_char( "Done.\n\r", ch );
	save_deity( deity );
	return;
    }

    if ( !str_cmp( arg2, "sdtrap" ) )
    {
	deity->sdtrap = atoi( argument );
	send_to_char( "Done.\n\r", ch );
	save_deity( deity );
	return;
    }
...
void do_showdeity( CHAR_DATA *ch, char *argument )
{
    ...
    deity->name, deity->filename, deity->detitle, deity->domains, deity->description );   
	...
	( deity->race2 < 0 || deity->race2 > MAX_RACE ) ? "none" : npc_race[deity->race2] );
    ch_printf( ch, "Objstat: %-9dWorshippers: %d\n\r", deity->objstat, deity->worshippers );
    ch_printf( ch, "Deityobj: %-12dAvatar: %-12dDerecall: %d\n\r",
		deity->deityobj, deity->avatar, deity->derecall );
	ch_printf( ch, "Affectednum: %-5dElementnum: %-6dSusceptnum: %d\n\r", 
		deity->affectednum, deity->elementnum, deity->susceptnum );
	...
	ch_printf( ch, "Suplift: %-9d Sdtrap: %-9d\n\r",
	deity->suplift, deity->sdtrap );
    return;
...
void do_deities( CHAR_DATA *ch, char *argument )
{
	...
	send_to_pager_color( "Deity         Worshippers      Domains\n\r", ch );
	for ( deity = first_deity; deity; deity = deity->next )
	{
	   pager_printf_color( ch, "&G%-14s    &g%-5d        %s\n\r", deity->name, deity->worshippers, deity->domains );
	   count++;
	}
	if ( !count )
	{
    ...
    pager_printf_color( ch, "&gDeity:   &G%s  %s\n\r", deity->name, deity->detitle );
    pager_printf_color( ch, "&gDomains:   &G%s\n\r", deity->domains );

    pager_printf_color( ch, "&gAlignment: &G%+4.4d  &gWorshippers:  &G%d\n\r",
            deity->alignment, deity->worshippers);
   
	pager_printf_color( ch, "&gFavored Race: &G%-14.14s &gHated Race: &G%-14.14s\n\r", 
	( deity->npcrace < 0 || deity->npcrace > MAX_NPC_RACE ) ? "none" : npc_race[deity->npcrace],
	( deity->npcfoe < 0 || deity->npcfoe > MAX_NPC_RACE ) ? "none" : npc_race[deity->npcfoe] );
   
    pager_printf_color( ch, "&gRace: &G%s %s&g Class: &G%s&g Sex: &G%s\n\r", 
	( deity->race < 0 || deity->race > MAX_RACE ) ? "any" : npc_race[deity->race],
	( deity->race2 < 0 || deity->race2 > MAX_RACE ) ? "" : npc_race[deity->race2],
	( deity->class < 0 || deity->class > MAX_CLASS ) ? "any" : npc_class[deity->class],
	  deity->sex == -1	  ? "any"   :
              deity->sex == SEX_MALE    ? "male"   :
              deity->sex == SEX_FEMALE  ? "female" : "neutral");
...
void do_supplicate( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = last_object; obj; obj = obj->prev )
	{
	   ...
	   && (obj->pIndexData->vnum == 11 ) )
	   {
		...
		|| IS_SET( obj->in_room->room_flags, ROOM_DEATH ) 
		|| (obj->timer > 35) )
		{
   ...
   if ( !str_cmp( arg, "avatar" ) )
   {
	...
	if ( ch->pcdata->deity->avatar < 1 || get_mob_index( ch->pcdata->deity->avatar ) == NULL )
	{
	...
	victim->hit = ch->level*20 + ch->pcdata->favor;
	victim->alignment = ch->pcdata->deity->alignment;
	victim->max_hit = ch->level*20 + ch->pcdata->favor;
 ...
 if ( !str_cmp( arg, "object" ) )
   {
	...
	if ( ch->pcdata->deity->deityobj < 1 || get_obj_index( ch->pcdata->deity->deityobj ) == NULL )
		{
			...
			sprintf( buf, "holy symbol faith sigil %s", ch->pcdata->deity->name );
			STRFREE( obj->name );
			obj->name = STRALLOC( buf );
 
			sprintf( buf, "a holy symbol of %s", ch->pcdata->deity->name );
			STRFREE( obj->short_descr );
			obj->short_descr = STRALLOC( buf );
 
			sprintf( buf, "A holy symbol of %s has fallen here.", ch->pcdata->deity->name );
   ...
   if ( !str_cmp( arg, "uplift" ) )
   {
      if (ch->pcdata->favor < ch->pcdata->deity->suplift ) {
      ...
      {
        send_to_char( "You must become downtrodden before being eligible for uplifting.\r\n",ch);
        return;
      }
      ch->hit+= 10 + ch->level;
      ch->pcdata->favor -= ch->pcdata->deity->suplift;
      ...
      act( AT_MAGIC, "$n is engulfed in holy light, and is partially healed!", ch, NULL, NULL, TO_ROOM );
      send_to_char("You are momentarily engulfed in holy light.\r\nYou have been healed by your God!\r\n",ch);
      return;
   }
   if ( !str_cmp( arg, "recall" ) )
   {
	...
	if (ch->position == POS_FIGHTING) {
	  send_to_char( "You cannot concentrate enough to supplicate!\r\n",ch);
	  return;
	}
	...
	if ( !IS_NPC(ch) && ch->pcdata->clan 
		&& (get_room_index(ch->pcdata->clan->recall ) != NULL))
      	  location = get_room_index( ch->pcdata->clan->recall );
	else
	if ((get_room_index(ch->pcdata->deity->derecall ) == NULL
		|| (ch->pcdata->deity->derecall < 1 )))
       		location = get_room_index( ROOM_VNUM_TEMPLE );
	else
       		location = get_room_index( ch->pcdata->deity->derecall );
	...
	if ( ch->pcdata->favor < ch->pcdata->deity->sdtrap )
	{
	   send_to_char( "You are not favoured enough for a deathtrap retrieval.\n\r", ch );
	   return;
	}
	...
	found = FALSE;
	sprintf(buf3,"the corpse of %s", ch->name);
	for ( obj = last_object; obj; obj = obj->prev )
	{
	   if ( obj->in_room
	   && !str_cmp( buf3, obj->short_descr )
	   && (obj->pIndexData->vnum == 11 ) )
	   {
		found = TRUE;
		if ( !IS_SET( obj->in_room->room_flags, ROOM_DEATH ) )
		{
		  act( AT_MAGIC, "Your corpse is not in a deathtrap!", ch, NULL, NULL, TO_CHAR );
		  return;
		}
		act( AT_MAGIC, "Your corpse appears suddenly, surrounded by a divine presence...", ch, NULL, NULL, TO_CHAR );		
		act( AT_MAGIC, "$n's corpse appears suddenly, surrounded by a divine force...", ch, NULL, NULL, TO_ROOM );
		obj_from_room(obj);
		obj = obj_to_room(obj, ch->in_room);
		ch->pcdata->favor -= ch->pcdata->deity->sdtrap; 
