...
"nosupplicate", "arena", "nomissile", "amplify", "locker", "prototype", "r31"
};
...
"missile", "back", "face", "ankle", "floating","r5","r6",
"r7","r8","r9","r10","r11","r12","r13"
};

char *	const	area_flags	[] =
{
"nopkill", "freekill", "noteleport", "hidden", "nosummon",
"denysummon", "nonpctele", "r7", "r8",
...
"berserk", "aqua_breath", "recurringspell", "contagious", "haste", "slow" };
...
"nstart", "immproof", "nobeep", "anonymous", "inductor", "married", "r15", "r16", "r17", "r18",
...
"flee" ,"autogold", "automap", "afk", "invisprompt", "married", "nomenu"
};
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  name short long description title spec clan\n\r", ch );
        send_to_char( "  council council2 quest qp qpa favor deity\n\r", ch );
	send_to_char( "\n\r",						ch );
	...
	send_to_char( "To toggle pkill flag: pkill\n\r",ch);
        send_to_char( "To set who person is wed to: wedto <name> (Need to be player)\n\r", ch );
        return;
    }
	...
	victim->height = URANGE( 10, value, 100 );
	return;
    }
    ...
    if ( !str_cmp( arg2, "weight" ) )
    {
        ...
        victim->weight = URANGE( 10, value, 500 );
        return;
    }
	...
	{
	    ch_printf( ch, "Blood range is -1 to %d.\n\r", MAX_LEVEL+10 );
	    return;
	}
    ...
    if( !str_cmp( arg2, "wedto" ) )
    {
        one_argument( argument, arg3 );
        if( IS_NPC( ch ) )
        {
            send_to_char( "You cannot do this on a mob.\n\r", ch );
            return;
        }
        if( ch->pcdata->wedto )
            STRFREE( ch->pcdata->wedto );
        ch->pcdata->wedto = str_dup( arg3 );
        send_to_char( "Done.\n\r", ch );
        return;
    }
...
void edit_buffer( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( cmd+1, "f" ) )
	{
	    ...
	    char *sptr = NULL,*lwptr = NULL,*wptr = NULL,*tptr = NULL;
...
/*		if ( get_trust(ch) >= sysdata.level_modify_proto  */
		...
		|| ( ch->pcdata->council2 && is_name( "aassign",
		ch->pcdata->council2->powers ))	)
...
void do_mlist( CHAR_DATA *ch, char *argument )
{
    ...
    for ( vnum = lrange; vnum <= trange; vnum++ )
    {
	  if ( (mob = get_mob_index( vnum )) == NULL )
	    continue;
	  pager_printf( ch, "%5d) %-20s %-3d %-12s %-15s %s\n\r", vnum,
					 mob->player_name,
					 mob->level, npc_class[mob->class],
					 npc_race[mob->race], mob->short_descr );
    }
}
