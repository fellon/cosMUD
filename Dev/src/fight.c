...
extern int arena_death(CHAR_DATA *ch, CHAR_DATA *victim); /* Scion arenacode in arena.c */
...
void violence_update( void )
{
	...
	||   IS_AFFECTED( ch, AFF_PARALYSIS ) )
        ...
        /* Scion arenacode */
        if ((ch->arena==2) && (sysdata.arena!=2)) {
                send_to_char("Your fight has been stopped until the betting is over!\r\n",ch);
                send_to_char("Your fight has been stopped until the betting in over!\r\n",ch);
	...
	if ( IS_AWAKE(ch) && ch->in_room == victim->in_room ) {
	    ...
	    /* haste affect -- Scion */
		if (IS_AFFECTED(ch, AFF_HASTE) && number_percent() < 50) 
			for (i=number_range(0, 5); i>0; i--)
...
ch_ret multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    chance = IS_NPC(ch) ? ch->level
	   : (int) ((LEARNED(ch, gsn_sixth_attack)+(dual_bonus*3.5))/5);
    if ( number_percent( ) < chance )
    {
    ...
    chance = IS_NPC(ch) ? ch->level
	   : (int) ((LEARNED(ch, gsn_seventh_attack)+(dual_bonus*4))/6);
    if ( number_percent( ) < chance )
    {
    ...
    chance = IS_NPC(ch) ? ch->level
	   : (int) ((LEARNED(ch, gsn_eighth_attack)+(dual_bonus*4.5))/7);
    if ( number_percent( ) < chance )
    {
...
ch_ret one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
	...
	/* slow affect -- Scion */
	if (IS_AFFECTED(ch, AFF_SLOW) && number_percent() < 50)
			return rNONE;
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if (!victim->name || victim->name[0]=='0') {
			bug("Bad victim in fight.c, line 1847");
			return rNONE;	
	}
 ...
 */
	...
	if ( damobj )
	{
		...
		set_cur_obj(damobj);
		/* Scion arenacode */
		if (!in_arena(victim)) /* Don't damage eq in arena */
			damage_obj(damobj);
	    }
    ...
    &&   victim->hit < 1 
    &&   !in_arena(victim)) /* Scion arenacode */
       victim->hit = 1;
...
*/
	...
	{
	    if (ch == victim)
			sprintf(log_buf, "%s has died!", victim->name);
		else
			sprintf( log_buf, "%s has been killed by %s!", victim->name,
		...
		gain_exp( victim, (exp_level(victim, victim->level) -
victim->exp)/4 );
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( (get_age( ch ) < 18 || ch->level < 5) && !in_arena(ch) ) /* Scion arenacode */
    {
    ...
    if ( (get_age( victim ) < 18 || victim->level < 5) && !in_arena(victim) ) /* Scion arenacode */
    {
	set_char_color( AT_WHITE, ch );
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( !IS_NPC( ch ) )
	{
	  ...
	  ch->pcdata->mkills++;
	  if (!IS_IMMORTAL(ch)) /* Scion 3/18/99 */
...
/*    &&    IS_SET( ch->pcdata->flags, PCFLAG_DEADLY )
	...
	int level_ratio;
		...
		adjust_hiscore( "mdeath", victim, victim->pcdata->mdeaths );
...
CHAR_DATA *who_fighting( CHAR_DATA *ch )
{
    ...
    if ( !ch->fighting || char_died(ch))
      return NULL;
	if (ch->next) {
		if (ch->next->prev != ch) {
			bug("who_fighting: ch->next->prev != ch. Shortcutting!!",0);
			ch->next->prev=ch;
			do_shout(ch, "Scion says to run for your lives!!");
		}
	}
	if (ch->prev) {
		if (ch->prev->next != ch) {
			bug("who_fighting: ch->prev->next != ch. Shortcutting!!",0);
			ch->prev->next=ch;
			do_shout(ch, "Scion says, 'PANIC!!'");
		}
	}
...
{	12,	/* Head */
	...
	OBJ_VNUM_EAR,		/* ear */
	OBJ_VNUM_EYE,		/* eye */
	OBJ_VNUM_LONG_TONGUE,	/* long_tongue */
	OBJ_VNUM_EYESTALKS,	/* eyestalks */
	OBJ_VNUM_TENTACLES,	/* tentacles */
	OBJ_VNUM_FINS,		/* fins */
	...
	OBJ_VNUM_SHARPSCALES,	/* sharpscales */
	OBJ_VNUM_BEAK,		/* beak */
	OBJ_VNUM_HAUNCHES,	/* haunches */
	OBJ_VNUM_HOOVES,	/* hooves */
	OBJ_VNUM_PAWS,		/* paws */
	OBJ_VNUM_FORELEGS,	/* forelegs */
...
void new_dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, OBJ_DATA *obj )
{
	...
	sprintf( buf2, "You %s $N%c &R[&W%i&R]&W", vs, punct, dam );
	sprintf( buf3, "$n %s you%c &R[&W%i&R]&W", vp, punct, dam );
    }
    else
	...
	/* Rathkor's posison damage fix	-Marten */
	...
	sprintf( buf2, "Your %s %s $N%c &R[&Y%d&R]&W",  attack, vp, punct,
dam );
	sprintf( buf3, "$n's %s %s you%c &R[&Y%d&R]&W", attack, vp, punct,
dam );
    }
...
/*    char buf[MAX_STRING_LENGTH]; */
    ...
    /* Scion arenacode */
    if (in_arena(ch)) {
	send_to_char( "You do not need to MURDER a player in the arena!\r\n", ch);
	return;
    }
...
*/
    ...
    WAIT_STATE( ch, 1 * PULSE_VIOLENCE );
/*    sprintf( buf, "Help!  I am being attacked by %s!",  Spammy crap.. let 'em yell for themselves -- Scion 
                   IS_NPC( ch ) ? ch->short_descr : ch->name );
    if ( IS_PKILL(victim) )
...
void do_panic( CHAR_DATA *ch, char *argument )
{
    ...
    do_yell(ch,"HELP ME... arrg arrg ack ack ... aaaayeeeeaaaahhh"); /* Fellon */
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC( ch ) )
	{
	    ...
	    los = ( exp_level( ch, ch->level+1 ) - exp_level( ch,
ch->level ) ) * 0.0015;
	    if ( ch->level < LEVEL_AVATAR )
	    {
	      sprintf(buf, "You flee head over heels from combat, losing %d experience.", los );
    ...
    los = ( exp_level( ch, ch->level+1 ) - exp_level( ch, ch->level ) ) *
0.005;
    if ( ch->level < LEVEL_AVATAR )
    {
      sprintf(buf, "You attempt to flee from combat, losing %d experience.\n\r", los );
      act( AT_FLEE, buf, ch, NULL, NULL, TO_CHAR );
    }
    else
