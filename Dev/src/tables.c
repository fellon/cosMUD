...
DO_FUN *skill_function( char *name )
{
	...
	if ( !str_cmp( name, "do_ax_export" ))		return do_ax_export;
	if ( !str_cmp( name, "do_ax_import" ))		return do_ax_import;
	break;
    case 'b':
	...
	if ( !str_cmp( name, "do_buylocker" ))		return do_buylocker;
	break;
    case 'c':
	...
	if ( !str_cmp( name, "do_counciltalk" ))	return do_counciltalk;
 	if ( !str_cmp( name, "do_counciltalk2" ))	return do_counciltalk2;
	...
	if ( !str_cmp( name, "do_extraname" ))		return do_extraname;
	...
	if ( !str_cmp( name, "do_invis" ))		return do_invis;
        ...
        if ( !str_cmp( name, "do_iscore" ))             return do_iscore;
        if ( !str_cmp( name, "do_isetup" ))		return do_isetup;
	if ( !str_cmp( name, "do_istats" ))		return do_istats;
#ifdef I3  /* Samson 12-26-00 */
	if ( !str_cmp( name, "do_i3" ))			return do_i3;
	if ( !str_cmp( name, "do_i3who" ))		return do_i3who;
	if ( !str_cmp( name, "do_i3finger" ))		return do_i3finger;
	if ( !str_cmp( name, "do_i3locate" ))		return do_i3locate;
	if ( !str_cmp( name, "do_i3tell" ))		return do_i3tell;
	if ( !str_cmp( name, "do_i3reply" ))		return do_i3reply;
	if ( !str_cmp( name, "do_i3emote" ))		return do_i3emote;
	if ( !str_cmp( name, "do_i3beep" ))		return do_i3beep;
#endif
	break;
    case 'k':
...
/*	if ( !str_cmp( name, "do_lookmap" ))		return do_lookmap; */
	...
	if ( !str_cmp( name, "do_makelocker" ))		return do_makelocker;
...
/*	if ( !str_cmp( name, "do_mapout" ))		return do_mapout; */
	...
	if ( !str_cmp( name, "do_mdelete" ))		return do_mdelete;
        if ( !str_cmp( name, "do_memorize" ))           return do_memorize;
	...
	if ( !str_cmp( name, "do_mpqpadd" ))		return do_mpqpadd;
	if ( !str_cmp( name, "do_mpmarry" ))		return do_mpmarry;
	if ( !str_cmp( name, "do_mpdivorce" ))		return do_mpdivorce;
...
/*	if ( !str_cmp( name, "do_opentourney" ))	return do_opentourney; */
	...
	if ( !str_cmp( name, "do_panic" ))		return do_panic;
	...
	if ( !str_cmp( name, "do_staff" ) )		return do_staff;
	...
	if ( !str_cmp( name, "do_tech" ))		return do_tech;
...
/* Scion arenacode */
    ...
    if ( skill == do_ax_export )		return "do_ax_export";
    if ( skill == do_ax_import )		return "do_ax_import";
    ...
    if ( skill == do_buylocker )	return "do_buylocker";
    ...
    if ( skill == do_counciltalk2)	return "do_counciltalk2";
	...
	if ( skill == do_extraname )	return "do_extraname";
    ...
    if ( skill == do_iscore )           return "do_iscore";
...
/*    if ( skill == do_lookmap )		return "do_lookmap"; */
    ...
    if ( skill == do_makelocker )	return "do_makelocker";
...
/*    if ( skill == do_mapout )		return "do_mapout"; */
    ...
    if ( skill == do_memorize)          return "do_memorize";
	...
	if ( skill == do_mpqpadd )		return "do_mpqpadd";

	if ( skill == do_mpmarry )		return "do_mpmarry";

	if ( skill == do_mpdivorce )	return "do_mpdivorce";
...
/*  if ( skill == do_opentourney )	return "do_opentourney"; */
    ...
    if ( skill == do_panic )		return "do_panic";
    ...
    if ( skill == do_staff )		return "do_staff";
    ...
    if ( skill == do_tech )		return "do_tech";
...
#ifdef I3 /* Section added by Samson 12-26-00 */
    if ( skill == do_i3 )		return "do_i3";
    if ( skill == do_i3who )		return "do_i3who";
    if ( skill == do_i3finger )	return "do_i3finger";
    if ( skill == do_i3locate )	return "do_i3locate";
    if ( skill == do_i3tell )		return "do_i3tell";
    if ( skill == do_i3reply )	return "do_i3reply";
    if ( skill == do_i3emote )	return "do_i3emote";
    if ( skill == do_i3beep )		return "do_i3beep";
#endif
    sprintf( buf, "(%p)", skill );
    return buf;
}
...
bool load_class_file( char *fname )
{
	...
	case 'N':
		...
		KEY( "Nanny",	class->nanny,		fread_number( fp ) );	/* added for nanny sort */
...
void write_class_file( int cl )
{
	...
	fprintf( fpout, "Nanny		 %d\n",		class->nanny		);	/* added for nanny sorting */
...
bool load_race_file( char *fname )
{
	...
	case 'N':
		...
		KEY( "Nanny",	race->nanny,		fread_number( fp ) );	/* added for nanny sort */
	...
	case 'S':
		KEY( "Species",	race->species,		fread_number( fp )	);	/* species snip 5/28/02 */
