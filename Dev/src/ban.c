...
syntax_message:
	...
	send_to_char("    Where field is site, race or class and number is the ban number.\n\r", ch);
        send_to_char("    No arguments lists the current bans.\n\r", ch );
	send_to_char("    Duration is how long the ban lasts in days.\n\r", ch );
        send_to_char("    Type is one of newbies, mortals, all, or Warn.\n\r", ch );
...
void dispose_ban ( BAN_DATA *pban , int type)
{
  ...
  tmp_first = type == BAN_SITE ? first_ban :
    (type == BAN_CLASS ? first_ban_class : first_ban_race);
  tmp_last = type == BAN_SITE ? last_ban : 
    (type == BAN_CLASS ? last_ban_class : last_ban_race);

  UNLINK( pban, tmp_first, tmp_last, next, prev );
  free_ban( pban );
  return;
}
