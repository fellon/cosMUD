/* $Id: imc.c,v 1.18 2000/02/26 14:17:16 gmoody Exp $ */
/*
...
/* log and announce if closed via event - shogar 2/26/2000 */
  imc_logstring("%s: closing link", shorthost);
...
static void do_close(imc_connect *c)
{
  ...
  {
        /* changed event, dont announce a simple reboot  - shogar - 2/2/2000 */
  	imc_add_event(60, ev_close_notify, c->info->name, 1);
//  	imc_close_notify(name);
  ...
  if (!c->info || !(c->info->flags & IMC_QUIET))
  {
  	name=imc_getconnectname(c);
    	...
    	if(name
  	&& !imc_find_event(ev_close_notify,c))
...
static void forward(imc_packet *p)
{
  ...
  if (!strcasecmp(imc_mudof(p->i.to), imc_name))
    return;
...
static void clientpassword(imc_connect *c, const char *argument)
{
  ...
  imc_cancel_event(ev_close_notify, c->info->name);
}
...
void imc_startup_network(void)
{
  ...
  imc_stats.txcompressed_bytes = 0;
  imc_stats.rxcompressed_bytes = 0;
...
int lzo_decompress( const lzo_byte *src, lzo_uint src_len, lzo_byte *dst, lzo_uint *dst_len, lzo_voidp wrkmem )
{
    lzo_byte compinstr[IMC_DATA_LENGTH + IMC_DATA_LENGTH / 64 + 16 + 4];
    char *dd = compinstr;

    if( !src || src_len <= 0 || src_len > (IMC_DATA_LENGTH+IMC_DATA_LENGTH/64+16+4) )
    {
        // Set dst to src?
        strcpy( dst, src );
        return FALSE;
    ...
    *dst=0;
    ...
    lzo1x_decompress( compinstr, src_len, dst, dst_len, wrkmem );

    dst[*dst_len]=0;

    imc_stats.rxcompressed_bytes += src_len;
    imc_stats.rxexpanded_bytes += *dst_len;
...
int lzo_compress( const lzo_byte *src, lzo_uint src_len, lzo_byte *dst, lzo_uint *dst_len, lzo_voidp wrkmem, char *outcoded )
{
    ...
    if( *dst_len > src_len )
        return FALSE; // Sending out more data compressed than otherwise
    ...
    *d=0;
    ...
    imc_stats.txcompressed_bytes += *dst_len;
    imc_stats.txexpanded_bytes += src_len;
