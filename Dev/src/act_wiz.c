...
/* Replaced with new wizhelp function by Scion 3/30/99 */
/*
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
 ...
 END COMMENT */
    ...
    void do_wizhelp( CHAR_DATA *ch, char *argument )
    {
        ...
        col = 0;
        set_pager_color( AT_PLAIN, ch );
     for(level = LEVEL_IMMORTAL - 1; level <= LEVEL_SUPREME + 1; level++ )
       {
        pager_printf(ch, "&g[&WLevel %d&g]&c\n\r", level); 
       for ( hash = 0; hash < 126; hash++ )
    	for ( cmd = command_hash[hash]; cmd; cmd = cmd->next )
    	    if ( cmd->level >= LEVEL_HERO
    	    &&   cmd->level <= get_trust( ch ) 
...
void do_bamfin( CHAR_DATA *ch, char *argument )
{
	...
	if (strlen(argument) > 80) {
		send_to_char("Your bamfin must be under 80 characters.\r\n",ch);
		return;
	}
...
void do_bamfout( CHAR_DATA *ch, char *argument )
{
	...
	if (strlen(argument) > 80) {
		send_to_char("Your bamfout must be under 80 characters.\r\n",ch);
		return;
	}
...
void do_forceclose( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch)) 
        send_to_char("NPCs should never be using commands like this!\r\n",ch);
...
void do_recho( CHAR_DATA *ch, char *argument )
{
    ...
    one_argument( argument, arg );
/* who cares what they like:) shogar  
    if ( !str_cmp( arg, "Thoric" )
    ||   !str_cmp( arg, "Circe" )
    ||   !str_cmp( arg, "Haus" )
    ||   !str_cmp( arg, "Scryn" )
...
void do_trans( CHAR_DATA *ch, char *argument )
{
    ...
    if(!IS_NPC(victim) && IS_SET(victim->pcdata->flags,PCFLAG_IMMPROOF))
    {
	if (get_trust(ch) >= sysdata.level_override_private)
		send_to_char("IMMPROOF flag overridden.\r\n", ch);
	else {
		send_to_char("They will have to CONFIG -IMMPROOF first\n\r",ch);
		return;
    	}
    }
...
/*    if ( !IS_NPC(victim) )
	pager_printf_color( ch, "&cStatus : &w%-10s", CAN_PKILL(victim) ? "Deadly" :
	    IS_PKILL(victim) ? "Pre-Deadly" : "Non-Deadly" ); -- Scion */
	...
	pager_printf_color( ch, "&cUser: &w%s@%s   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\n\r",
		...
		victim->trust, victim->pcdata->authed_by != NULL ?
    ...
    if ( !IS_NPC(victim) && (victim->pcdata->council || victim->pcdata->council2) )
	pager_printf_color( ch, "&cCouncil:  &R%s    &cCouncil2:  &R%s\n\r",
	  victim->pcdata->council_name, victim->pcdata->council2_name);
    ...
    send_to_pager( "\n\r", ch );
	if (victim->practice)
		pager_printf_color( ch, "&cPractice Sessions: &w%d\n\r", victim->practice);
    if (victim->arena_wins)
      pager_printf_color( ch, "&cArena Wins: &w%d\n\r", victim->arena_wins ); /* Scion arenacode */
	...
	victim->long_descr != NULL ? victim->long_descr : "(none set)\n\r" );
    else
    {
        if ( victim->short_descr != NULL )
          pager_printf_color( ch, "&cShortdesc  : &w%s\n\r",
	    victim->short_descr );
	if ( victim->long_descr != NULL )
...
void do_switch( CHAR_DATA *ch, char *argument )
{
    ...
    /* PLR_FREEZE check removes small but rather inconvenient bug --Prage */
    if ( !IS_NPC(victim) && (ch->level < LEVEL_GREATER || xIS_SET(victim->act, PLR_FREEZE)) )
    {
	send_to_char( "You cannot switch into that player!\n\r", ch );
	return;
    }
	if ( ch->level <= victim->level ) {
		send_to_char( "Your victim must be a lower level than you!\r\n", ch);
		return;
	}
  ...
  /*You can demote yourself but not someone else at your own trust.-- Narn*/
        ...
        else
	{
     ...
     /* Stuff added to make sure character's wizinvis level doesn't stay
        higher than actual level, take wizinvis away from advance < LEVEL_AVATAR */
...
/* let's find out who is using this, promoting commands should always be 
   logged - shogar */
    sprintf(arg,"elevate issued by %s on %s",ch->name,argument);
    log_string(arg);
...
void do_restore( CHAR_DATA *ch, char *argument )
{
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
		...
		vch->pcdata->condition[COND_FULL] = 40; /* Fellon add */
		vch->pcdata->condition[COND_THIRST] = 40; /* Fellon add */
		update_pos (vch);
...
void do_users( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "\n\r&CDesc&w|&cCon&w|&GIdle&w| &gPort &w| &YPlayer      &O@HostIP           ");
    if ( get_trust(ch) >= LEVEL_GOD)
     strcat(buf, "&w| &RUsername");
    strcat(buf, "\n\r");
    strcat(buf, "&w----+---+----+------+-------------------------------");
    if ( get_trust(ch) >= LEVEL_GOD)
      strcat(buf, "&w+---------");
	...
	||   (d->character && can_see( ch, d->character )) )
	{
	    count++;
	    sprintf( buf,
	     " &C%3d&w| &c%2d&w|&G%4d&w|&g%6d&w| &Y%-12s&O@%-16s ",
		d->descriptor,
		d->connected,
		d->idle / 4,
		d->port,
		d->original  ? d->original->name  :
		d->character ? d->character->name : "&B(none)&w",
		d->host);
	    if (get_trust(ch) >= LEVEL_GOD)
	      sprintf( buf + strlen( buf ), "&w| &R%s", d->user );
	    strcat(buf, "\n\r");
	    send_to_pager( buf, ch );
	}
      }
	...
	||   ( d->character && !str_prefix( arg, d->character->name ) ) ) )
	{
		...
		d->port,
		d->original  ? d->original->name  :
		d->character ? d->character->name : "&B(none)",
		d->host
		);
	    buf[0] = '\0';
	    if (get_trust(ch) >= LEVEL_GOD)
	      sprintf(buf, "&w| &R%s", d->user);
	    strcat(buf, "\n\r");
	    send_to_pager( buf, ch );
	}
      }
...
void do_force( CHAR_DATA *ch, char *argument )
{
    ...
    /* This makes frozen players forcable --Prage */
    ...
    if(wasfroze) xSET_BIT(victim->act, PLR_FREEZE);
    }
...
void do_newbieset( CHAR_DATA *ch, char *argument )
{
	...
	return;
     ...
     else if ( (victim->class == 3) || (victim->class == 6) || (victim->class == 8) )
     {
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !rename( buf, buf2 ) )
  {
    ...
    {
      ch_printf( ch, "Unknown error #%d - %s (immortal data).  Report to Scion.\n\r",
              errno, strerror( errno ) );
      sprintf( buf2, "%s destroying %s", ch->name, buf );
      perror( buf2 );
    }
  ...
  {
    set_char_color( AT_WHITE, ch );
    ch_printf( ch, "Unknown error #%d - %s.  Report to Scion.\n\r",
            errno, strerror( errno ) );
...
/*	pager_printf_color(ch, "\n\r&wPfile autocleanup status: &W%s  &wDays before purging newbies: &W%d\n\r",
    ...
    pager_printf_color(ch, "&wPrivate && immproof override:   &W%-2d\n\r", sysdata.level_override_private);
    ...
    /* Scion arenacode */
    pager_printf_color(ch, "  &wImmortals allowed in arena: &W%s\n\r", (sysdata.imm_arena==TRUE) ?"&GYES&w" : "&RNO&w" );

	pager_printf_color(ch, "  &wCrashGuard: &W%s\n\r", (sysdata.crashguard==TRUE) ? "&GActive&w" : "&RInactive&w" );
        pager_printf_color(ch, "  &wWebServer: &W%s", (sysdata.webtoggle==TRUE) ? "&GActive&w" : "&RInactive&w" );
	pager_printf_color(ch, "  &wWebPort: &W%d\n\r", sysdata.webport );
    return;
  }
 ...
 /* if ( !str_cmp( arg, "pfiles" ) )
  ...
  if (!str_prefix( arg, "club_overseer" ) )
  {
  ...
  if (!str_prefix( arg, "club_advisor" ) )
  {
...
/*  if ( !str_cmp(arg, "newbie_purge"))
  ...
  if (!str_cmp(arg, "webport"))
  {
    sysdata.webport = level;
    send_to_char("Ok.\n\r", ch);
    return;
  }
  ...
  if ( !str_cmp( arg, "imc_level") || !str_cmp( arg, "imc_mail_level"))
  {
    sysdata.imc_mail_level = level;
    send_to_char( "Ok.\n\r", ch);
    return;
  }
...
/* Scion arenacode */
  else if (!str_cmp(arg, "imm_arena")) {
    send_to_char(((sysdata.imm_arena==TRUE) ? ("Imm arena ILLEGAL.\r\n") : ("Imm arena ALLOWED.\r\n")), ch);
    if (sysdata.imm_arena==TRUE) {
      do_info(ch, "Immortals are no longer allowed in the arena.");
      sysdata.imm_arena=FALSE;
    } else {
      do_info(ch, "Immortals are now allowed in the arena!");
      do_info(ch, "Now\'s your chance to show \'em who\'s boss!");
      sysdata.imm_arena=TRUE;
    }
  }
  ...
  else if (!str_cmp(arg, "webserver")) {
          if (sysdata.webtoggle==TRUE) {
                  send_to_char("WebServer disabled.\r\n", ch);
                  sysdata.webtoggle=FALSE;
          } else {
                  send_to_char("WebServer enabled.\r\n", ch);
                  sysdata.webtoggle=TRUE;
          }
  }
...
/* must be rod a thing only one hell here. -shogar */
/*
...
void do_showclass( CHAR_DATA *ch, char *argument )
{
    ...
    pager_printf_color( ch, "&wBase Alignment : &W%-2d\n\r",
	class->alignment ); /* Scion 3/20/99 */
    if ( arg2[0] != '\0' )
    {
	int x, y, cnt;
...
void do_setclass( CHAR_DATA *ch, char *argument )
{
    ...
    {
	class->alignment = atoi( argument );
	if (class->alignment>1000)
	    send_to_char("Warning: max alignment is 1000.\n\r",ch);
	else if (class->alignment<-1000)
	    send_to_char("Warning: min alignment is -1000.\n\r",ch);
	send_to_char( "Done.\n\r", ch );
	return;
    }
...
void do_setrace( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
        ...
        send_to_char( "  mana_regen hp_regen base_age         \n\r",     ch );
        return;
    }
	...
	if ( !str_cmp( arg2, "base_age" ) ) {
		race->base_age = atoi( argument );
		send_to_char("Done.\n\r", ch );
		return;
	}
...
void do_qpset( CHAR_DATA *ch, char *argument )
{
  ...
  if ( nifty_is_name_prefix( arg2, "give" ) )
  {
    ...
    || str_cmp(ch->pcdata->council2_name, "Quest Council"))
...
"---|-------------|----------------------|--------------------------|-----------\n\r");
   ...
   if( !str_cmp( arg, "add" ) )
   {
	  ...
	  ||  str_cmp(ch->pcdata->council2_name, "Code Council")))
...
/* Classes Function Added by Fellon */
...
/* Races Function added By Fellon */
...
/*
 * This function is in short for dealing with wimps and wingers.
 * It sets the PLR_WIMP flag (not yet existant) and PLR_FLEE.
 * The PLR_WIMP flag should have the affect of makeing kill/murder and
 * offensive spells not work. As well as a few screams for help when attacked.
 * --Prage
 */

/*
void do_wimp(CHAR_DATA *ch, char argument)
{
	char arg[MAX_STRING_LENGTH];
	CHAR_DATA *victim;
	
	if(IS_NPC(ch) || get_trust(ch) < LEVEL_ACOLYTE) {
	...
	argument = one_argument( argument, arg);
	if(arg[0] == '\0') {
		send_to_char("Who's the wimp?\n\r", ch);
		return;
	}
	
	if(!(victim = get_char_world(ch, arg)) ) {
		send_to_char("They could not be found.\n\r", ch);
		return;
	}
	
	if(IS_NPC(victim)) {
		send_to_char("Only found a mob.\n\r", ch);
		return;
	}
	...
	if(victim->level >= LEVEL_NEOPHYTE) {
		if(ch->level < victim->level) {
			...
			send_to_char("Wouldn't it be better just to slay them?\n\r", ch);
		}
		return;
	}
	
	if( xIS_SET(victim->act, PLR_WIMP)) {
		xREMOVE_BIT(victim->act, PLR_WIMP);
		xREMOVE_BIT(victim->act, PLR_FLEE);
		send_to_char("They are no longer considered wimps.\n\r", ch);
	} else {
		xSET_BIT(victim->act, PLR_WIMP);
		xSET_BIT(victim->act, PLR_FLEE);
		send_to_char("They are now true wimps.\n\r", ch);
	}
}
*/
