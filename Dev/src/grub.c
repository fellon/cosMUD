     ...
     CHAR_DATA *ch, int op_num, bool or_sw, int dis_num)
{
  ...
  "Ven", "Inc", "Baa", "Rol"};
  char council[][4] =
  {"   ", "Bld", "Cod", "HiC", "Rep", "Res", "PR ", "QC ", "Cod", "AC ",
   "Sym"};

  if ( ( fp = fopen( "../system/grub.dat", "r" ) ) == NULL )
  {
  ...
  if ( obj_index_hash[hash] )
     for (pObj=obj_index_hash[hash]; pObj; pObj=pObj->next)
         if (pObj->layers > 0)
         {
            ...
            pager_printf(ch, "%4d %8d %-15s %5d   %s\n\r",
              cou, pObj->vnum, flag_string(pObj->wear_flags,w_flags), pObj->layers,
              pObj->short_descr);
          }
}
