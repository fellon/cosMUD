/* $Id: icec.c,v 1.12 2000/02/03 13:56:20 gmoody Exp $ */
/*
...
/* Decompression/Compression wrappers */
void lzo_decompress( const lzo_byte *src, lzo_uint src_len, lzo_byte *dst, lzo_uint *dst_len, lzo_voidp wrkmem );
int lzo_compress( const lzo_byte *src, lzo_uint src_len, lzo_byte *dst, lzo_uint *dst_len, lzo_voidp wrkmem, char *outcoded );
...
int icec_recv(const imc_packet *p, int bcast)
{
    ...
    else if( !strcasecmp( p->type, "ice-comp-msg-r" ) )
    {
        icec_recv_msg_comp_r( p->from,
                              imc_getkey( &p->data, "realfrom", "" ),
                              imc_getkey( &p->data, "channel", "" ),
		     ...
		     int emote)
{
  ...
  if (!c->local || (c->policy!=ICE_PRIVATE && c->policy != ICE_CPRIVATE))
    return;
...
void icec_recv_msg_comp_r( const char *from,
                           const char *realfrom,
                           const char *chan,
                           int length,
                           const char *txt,
...
{
    ice_channel *c;
    lzo_byte newtxt[IMC_DATA_LENGTH + IMC_DATA_LENGTH / 64 + 16 + 4];
    const char *mud;
    lzo_uint new_len = 0;
    ...
    lzo_decompress(txt, length, newtxt, &new_len, NULL );

    /*  we assume that anything redirected is automatically audible - since we
     *  trust the ICEd
     */

    icec_showchannel(c, realfrom, newtxt, emote);
}
                          ...
                          int emote)
{
    ...
    icec_showchannel(c, from, newtxt, emote);
}
...
/* took out for 2.00 - shogar - 1/25/2000 */
...
void icec_sendmessage(ice_channel *c, const char *name, const char *text, int emote)
{
    ...
    /* commpress policy - shogar - 1/29/2000 */
    if(c->policy == ICE_COPEN || c->policy == ICE_CPRIVATE)
    {
        inlen = strlen( text );
        compressit = lzo_compress( text, inlen, compoutstr, &compoutlen,
                                   wrkmem, outcoded );
    }
    ...
    if (c->policy == ICE_PRIVATE || c->policy == ICE_CPRIVATE)
    {
        ...
        strcpy(out.type, compressit ? "ice-comp-msg-p" : "ice-msg-p");
        sprintf(out.to, "ICE@%s", ice_mudof(c->name));
    }
    else
    {
        /* broadcast */
        strcpy(out.type, compressit ? "ice-comp-msg-b" : "ice-msg-b");
        strcpy(out.to, "*@*");
    }
