...
/* Marries two players -- 5/8/99 Scion */

void do_mpmarry( CHAR_DATA *ch, char *argument ) {
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		...
		progbug("Mpmarry: missing argument",ch);

		send_to_char("Mpmarry whom?\r\n",ch);
	...
	if ( ( bride = get_char_room( ch, arg1 ) ) == NULL )

    {

        progbug( "Mpmarry: bride not present", ch );
	...
	if ( ( groom = get_char_room( ch, arg2 ) ) == NULL )

	{

		progbug( "Mpmarry: groom not present", ch);
	...
	if (xIS_SET( bride->act, PLR_MARRIED)) {

		send_to_char("You are already married!  Get a divorce first!", bride);

		send_to_char("Your bride-to-be is already married!  Have them get a divorce first!\r\n", groom);
	...
	if (xIS_SET( groom->act, PLR_MARRIED)) {

		send_to_char("You are already married!  Get a divorce first!", groom);

		send_to_char("Your groom-to-be is already married!  Have them get a divorce first!\r\n", bride);
	...
	xSET_BIT( bride->act, PLR_MARRIED );

	xSET_BIT( groom->act, PLR_MARRIED );

}	



/* Divorces two players -- 5/8/99 Scion */

void do_mpdivorce( CHAR_DATA *ch, char *argument ) {
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		...
		progbug("Mpdivorce: missing argument",ch);

		send_to_char("Mpdivorce <player1> <player2>\r\n", ch);
...

    {
        ...
        progbug( "Mpdivorce: char not present", ch );
	...
	if (!xIS_SET( bride->act, PLR_MARRIED)) {
		...
		send_to_char("You are not married!  Get married first!", bride);

		send_to_char("Your partner is not married!  Have them get married first!", groom);
	...
	if (!xIS_SET( groom->act, PLR_MARRIED)) {
		...
		send_to_char("You are not married!  Get married first!", groom);

		send_to_char("Your partner is not married!  Have them get married first!", bride);
	...
	xREMOVE_BIT( bride->act, PLR_MARRIED );

	xREMOVE_BIT( groom->act, PLR_MARRIED );

}



/* Adds glory to a player -- 5/8/99 Scion */

void do_mpqpadd( CHAR_DATA *ch, char *argument ) {
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' ) {
		...
		progbug("Mpqpadd:  missing argument", ch );

		send_to_char("Mpqpadd how much glory to whom?\r\n",ch);
...

    {
        ...
        progbug( "Mpqpadd: victim not present", ch );
...

    {
        ...
        progbug( "Mpqpadd:  trying to add glory to NPC", ch );
	...
	if (victim->pcdata->quest_curr + amount <= 0 )
		...
		victim->pcdata->quest_curr = 0;

	if (victim->pcdata->quest_accum + amount <= 0 )

		victim->pcdata->quest_accum = 0;
	...
	if (amount > 0)
		...
		ch_printf( victim, "Your glory has been reduced by %d.\n\r", (amount*-1) );



	if (amount > 10 || amount < -10) {

		sprintf(arg1, "Mpqpadd:  %d glory added to %s from mob #%d", amount, victim->name, ch->pIndexData->vnum);
...
void do_mpforce( CHAR_DATA *ch, char *argument )
{
	...
	&& IS_IMMORTAL( victim ) )
	{
	    sprintf(arg, "Mpforce - Mob #%d in room %d attempting to force link dead immortal",ch->pIndexData->vnum, ch->in_room->vnum);
		progbug( arg, ch );
	    return;
	}
    ...
    */
	...
	if ( !npcvict )
	{
		if (ch == victim)
		    ...
		    sprintf( log_buf, "%s has been killed by %s!",
		victim->name,
